...
GAIA keeps the wiki current and pruned as the project grows. Commits drive incremental updates; SPEC outcomes drain into the wiki at PR merge; a redundancy and staleness pass runs automatically whenever the wiki grows. The result: the knowledge base reflects the actual codebase, not the state it was in six months ago.
