/**
 * Dispatch-time coaching injection (UAT-033/034).
 *
 ...
 *
 * UAT-034: in the empty-string case, callers' prepend logic noops, so
 * the resulting system prompt is byte-identical to the non-mentorship
 * path.
 *
 * UAT-033: when an adaptation matches, the cache file
 * `.gaia/cache/coaching-active.txt` is written with content `1` so the
 * statusline 🧭 segment lights up (Phase 4 task-statusline-compass).
 * Cleared by `wiki-session-start.sh` at session boundary.
 *
 ...
 * with `COACHING=$(bin/gaia _internal-fetch-coaching ...)` and prepend
 * unconditionally; an empty result is the byte-identical UAT-034 path.
 *
 * Argv shape:
