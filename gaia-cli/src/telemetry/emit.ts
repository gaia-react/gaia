/* eslint-disable unicorn/prevent-abbreviations -- envelope/argv field names
   are frozen interface contracts in the SPEC-001 plan README. */
...
const parseAndValidate = (argv: readonly string[]): ValidationOutcome => {
  ...
  // UAT-010: unknown event type -> structured error, non-zero exit, no writes.
  ...
  // UAT-011: payload validation failure -> structured error, non-zero, no writes.
...
const writeStreams = async (args: WriteStreamsArgs): Promise<number> => {
  ...
  // Cloud emit (UAT-009: independent of mentorship opt-in). Projection runs
  // BEFORE either write (UAT-014 ordering); drift -> CLOUD_PROJECTION_DRIFT
  // and nothing lands.
 ...
 * exits via `process.exit` after this resolves. UAT-008 mandates silent
 * success — no stdout writes on the happy path.
 */
