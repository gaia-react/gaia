/* eslint-disable unicorn/prevent-abbreviations -- StorageRoots field names
   ...
   ensureMentorshipDirs) are frozen interface contracts in
   `.gaia/local/plans/spec-001-telemetry-v1/README.md` § "Frozen interface
   contracts" and § "Storage paths". Renaming would break Phase 2/3 tasks
   that bind to them. Internal-only locals follow the rule. */
/* eslint-disable no-bitwise -- POSIX file modes are bitfields; 0o777 masking
   is the standard idiom (UAT-003 verifies modes via & 0o777). */
 ...
 * Idempotent. Always called before a cloud emit (UAT-001).
 */
 ...
 * Creates the parent Claude project slug directory at mode 700 if absent (UAT-001 last clause).
