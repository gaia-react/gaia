...
- the `rm -rf` guard hook (`.claude/hooks/block-rm-rf.sh`) allowed the two most likely ways to write the dangerous command. It word-split each target but never removed quote characters, so a *quoted* `$HOME`, `/`, `.`, `.git`, or `node_modules` matched none of its patterns and was passed straight through, while the same target written bare was denied. The inversion is the sting: quoting the expansion is the careful, correct way to write the command, so the guard missed precisely the well-written form and caught only the sloppy one. Separately, the `--no-preserve-root` flag written *before* the recursive flag exited the hook's short-circuit before it ever reached its own unconditional deny for that flag, making the guard's first documented rule dead code in that flag order. Targets are now matched with their quotes stripped (including the partially-quoted form, where the quotes sit inside the token), and the flag is recognized in any position. This was always defense-in-depth rather than an open door, `settings.json` pre-approves only narrow scratch-path `rm` forms, so a bypassing command still met a normal permission prompt; the missing second layer is what is restored. The hook also gains the table-driven bats suite it never had, over the full deny/allow matrix in bare, double-quoted, and single-quoted shapes, whose first run is what surfaced the second bypass (#699)
