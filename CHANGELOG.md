...
- a SessionStart janitor (`.claude/hooks/local-janitor.sh`) that garbage-collects dead working-state residue under the gitignored `.gaia/local/` on every startup/resume. It removes only residue whose death is provable, orphaned merge-gate markers and audit re-run ledgers, completed-but-unswept plan dirs, stray empty dirs, and local `wiki-sync/*` landing branches whose upstream is gone, so it is a backstop, not an owner; any unprovable death is skipped and the hook always exits 0 so it can never block a session from starting. It also owns retention for the two local streams nothing else reaps, age-rotating stale cache artifacts and the `telemetry/cloud/` event log after 14 days (`gaia mentorship purge` deliberately leaves the cloud stream alone, so the janitor is its only reaper). Adds the `Local Working State` wiki concept page (#468, #506, #673)
...
- the mentorship opt-in moves from `/gaia-init` to `/setup-gaia`, so it is asked once during onboarding rather than during project initialization. `/gaia-init` no longer surfaces the prompt; `/setup-gaia`'s first run is where you choose whether to enable mentorship.
...
- `/gaia-plan`'s `plan_revised` mentorship event now attributes to the right SPEC: step 4.7 derived the spec id with `basename "$SPEC_PATH" .md`, which stripped `.md` off the `SPEC.md` filename and emitted the literal `--spec-id SPEC` on every revision. It now derives the id from the SPEC folder name, matching the token-tally derivation a few lines below (#537)
