...
- a SessionStart janitor (`.claude/hooks/local-janitor.sh`, invoked from `wiki-session-start.sh`) that garbage-collects dead working-state residue under the gitignored `.gaia/local/` on every `startup`/`resume`. It is the side-effect form of a SessionStart hook (it deletes files and injects nothing into context) and a backstop, not an owner, so each subsystem still self-prunes its own residue; the janitor removes only residue whose death is provable: orphaned `audit/<sha>.ok` and `<sha>.dispositions.json` merge-gate markers (a `<sha>` neither HEAD nor reachable from a local branch), completed-but-unswept `plans/<slug>/` dirs (a `RUNNING` sentinel naming a branch that no longer exists and is not `DEFERRED`/`PAUSED`/`PARKED`), and stray empty dirs. A git-scoped sweep, ordered ahead of the `.gaia/local` guard so a fresh clone is still swept, additionally hard-deletes any local `wiki-sync/*` landing branch whose upstream is `[gone]`: the auto-merge wiki landing flow (`gaia wiki chain finish` / `wiki sync land`) returns before its squash-merge completes and previously left those branches piling up as orphans, so both landing paths now add `--delete-branch` and return to the base branch to unblock the sweep. Fail-safe end to end: any unprovable death is skipped and the hook always exits 0 so it can never block a session from starting. Adds the `Local Working State` wiki concept page documenting the `.gaia/local/` layout and per-path retention, plus a `local-janitor` bats suite, and rebuilds the committed `gaia` / `gaia-maintainer` bundles (#468, #506)
- a forced-disposition gate on out-of-scope `code-review-audit` findings plus the `/gaia-debt` drain skill: debt the audit opens within its review radius but the PR did not change no longer routes through the gating Critical/Important/Suggestions sections, it gets a disposition of its own before the marker clears. Non-security out-of-scope findings file as deduped, severity-labeled `tech-debt` GitHub issues carrying a frozen versioned dedup key (`<!-- gaia-debt-key: v1 class=… path=… line=… -->`), a self-contained body passed via `--body-file` (never a `--body` argv that CI's `--verbose` would echo into the public Actions log), and a `Handler: prompt|plan` advisory class; filing is idempotent and the `tech-debt`/`severity:*`/`wontfix` labels are created on first use. Security-class findings are fail-safe classified (any Critical, any classless, secret-shaped, or from the security review dimension) and diverted off every public or enterprise-readable channel: a redacted operator surface locally, a count-only PR signal in CI on PUBLIC/INTERNAL repos, and a private issue only on a confirmed PRIVATE repo. The gate is the audit's fourth marker precondition, verified by re-querying open issues before the marker writes and backed at merge time by an `audit-disposition-check.sh` PreToolUse hook that denies `gh pr merge` only on a present-backend inconsistency and fails open otherwise. The `/gaia-debt` drain skill takes one `tech-debt` issue per invocation (deterministic severity-then-age ordering with no LLM evaluator, a fresh branch through the normal audit gate with `Closes #N`, and a drain-time security re-screen), surfaced by a `Run /gaia-debt (N issues)` statusline indicator whose count survives GitHub's eventual consistency (the refresher holds the staleness sentinel through a settle grace so a recompute fired the instant an issue closes cannot cache a stale count, and clears only once the count is consistent). The CI workflow grants the audit job `issues: write` and `Bash(gh:*)` so the same filing path runs from CI, and all maintainer-only path references that had leaked into adopter-shipped surfaces are scrub-wrapped or rephrased, with the bundle-time scrub's `maintainer-paths` scope extended to `.gaia/cli/templates/**`. Adds the `Audit Disposition and Debt Drain` wiki concept page. Deferred for v1: line-drift-tolerant dedup, a CI security-advisory credential mechanism, durable drain for diverted PUBLIC/INTERNAL security findings, and cross-band drain fairness (#465, #475, #495, #496)
...
- a React 19 idiom gate in the `react-code` skill (Gate 4): ref-as-prop over `forwardRef`, boolean-guarded `&&` over the numeric-0 leak (operand coercion is mandatory, with react-doctor's type-aware `rendering-conditional-render` rule as the pre-merge backstop for the general `count && <JSX/>` case that real-time lint's `.length` selector misses, so Claude coerces while authoring instead of deferring the fix), `use()` and the `<Context>` shorthand over `useContext`/`Context.Provider`, never `null` in render, and staying in React Router's lane for forms; `references/hook-patterns.md` gains `useEffectEvent` and ref-callback cleanup, and the `typescript` skill prefers `!!` over `Boolean()` for inline coercion (#441)
...
- onboarding is now a single `/setup-gaia` command, replacing `/setup-gaia-ci` and `/setup-cloned-gaia-project`. It detects your situation (fresh clone, first adopter, partial run, or fully provisioned), runs only the phases still owed, provisions the GitHub repository (create / adopt / manual, private by default), and is safe for any teammate to re-run after cloning. CI-token setup is out-of-band and never asks you to paste a secret into the chat: it reuses a token the repo or org already exposes when present, listing the reachable secret names at both repo and org scope (`actions/secrets` and `actions/organization-secrets`, which `gh secret list --repo` does not surface), by name only and never by value, and consuming an existing `CLAUDE_CODE_OAUTH_TOKEN` or `ANTHROPIC_API_KEY` automatically since the rendered workflows already wire both env vars; otherwise you set the secret yourself via `gh secret set` or the web UI and the command only verifies it exists by exact name (`gh secret list --json name`), never touching the value. Adds a repo-admin pre-gate, an in-place retry when the secret is absent, a `--reconfigure`-gated rotation path, and a safety rule that treats an accidentally-pasted token as compromised and mandates rotation. **Action required:** run `/setup-gaia` instead of the two retired commands; `/update-gaia` surfaces the removal of the old command files with a per-file confirm prompt on your next update (#478, #480, #509)
- `/gaia-init` setup-flow polish. It auto-detects the user's GitHub handle for `.github/CODEOWNERS` via `gh api user` (a single call that both verifies auth and returns the login) instead of always writing the `REPLACE-WITH-YOUR-GITHUB-HANDLE` placeholder: on success it writes `* @<handle>`, on failure (gh absent, unauthenticated, or empty) it keeps the placeholder and the required-follow-up warning; interactive mode offers the detected handle as an overridable default and HARD-BLOCKs only when detection fails, and the "never fabricate identity values" policy is updated to carve out this authenticated-identity case (there is no `git config` or remote-URL fallback). The i18n and CI prompts are polished too: the additional-languages question leads with the resolved primary and promotes "add more languages" to a first-class choice, free-text language entry uses a plain chat prompt (accepting freeform names / codes / native script resolved to ISO 639-1 with an echo-back confirmation) instead of the inert `AskUserQuestion` "Other" button, and the CI enable + run-mode questions carry `docs.gaiareact.com/maintenance/gaia-ci/` links. Finally, `/gaia-init` and `/setup-cloned-gaia-project` strip React Doctor's bundled extras after install (standalone Actions workflow, commit hook, `doctor` package script, pinned `react-doctor` devDependency), since GAIA already triggers react-doctor via the skill and the `code-review-audit` agent at `@latest` and the installer would otherwise write its hook into husky's generated `.husky/_/pre-commit`, bypassing the canonical `.husky/pre-commit`; the `add-locale` runbook points the LanguageSelect edit at the `LANGUAGE_LABELS` record and asserts the language-switch spec against a translated key (`cta`) (#454, #507)
...
- adopt `@gaia-react/lint` 1.8.0 (from 1.5.1), reconciling every lint delta it surfaces across the repo. The `no-null-render` autofix now rewrites a `return null` to `return undefined` inside render functions only (loaders, actions, and plain utils are never touched, and structural `null` such as DOM refs, `data(null)`, `Response.json(null)`, and Zod `.nullable()` is left intact), superseding the earlier flag-only never-`null` selectors and letting the now-redundant "never `null`" render mandate drop out of the `react-code` skill; the never-`null` convention had already reached GAIA-controlled types and state (the theme chain, `useComponentRect`'s timeout ref). A report-only `no-restricted-syntax` selector flags the most common numeric-0 leak, `.length && <JSX/>`. Earlier in the chain came the `resources+`/`actions+` `no-restricted-paths` UI-layer carve-out (retiring the inline disable in `ThemeSwitch`), the D-8 test-honesty rules (`vitest/prefer-called-with`, plus `no-restricted-imports` on `*.server` / `**/internals/**` from consumer tests), and the unicorn 65 batch. The bundled toolchain moves in step (`eslint-plugin-sonarjs` 4.0.3 to 4.1.0, `typescript-eslint` 8.61.1 to 8.62.0) and the findings the newer SonarJS rules surface are resolved (redundant optional-type annotations, more-specific test assertions, and a renamed `super-linear-regex` disable directive) (#433, #441, #461)
...
- routine dependency refresh across several waves: `@types/node` 25.9.3 → 26.0.0 (its major tracks the Node 26 type line; runtime stays pinned to Node 22.23.1 and the types are a superset, so no code changes), the `storybook` group to 10.4.6, `@playwright/test` to 1.61.0, `tailwindcss` to 4.3.1, `vitest` to 4.1.9, plus `@faker-js/faker` 10.5.0, `chromatic` 17.5.0, `happy-dom` 20.10.6, `nanoid` 5.1.14, and further minor/patch bumps. Transitive `undici` and the nested `vite` 7.x copy reached their patched security floors through the bumps, so those interim overrides were pruned, and the now-obsolete `qs` security-floor override (`'qs': '>=6.15.2'`) is dropped as well (`express` resolves `qs@6.15.2` on its own, so the pin is redundant and `pnpm audit` stays clean without it) (#387, #413, #415, #452)
- upgrade React Router 7.18.0 → 8.0.1 across the family (`react-router`, `@react-router/{node,serve,dev,fs-routes,remix-routes-option-adapter}`), a low-risk "boring major": GAIA already adopted all five v8 future flags on v7 and imports everything from `react-router`, so router runtime behavior is unchanged at cutover and the now-invalid `future` block in `react-router.config.ts` is deleted (middleware, route-module splitting, and the Vite Environment API are built-in defaults in v8). Also bumps `remix-utils` 9.3.1 → 10.0.0 and `remix-i18next` 7.5.0 → 8.0.0, which peer `react-router` ^8 natively (no pnpm override needed); v8 collapsed remix-i18next's subpath exports, so the middleware and client imports repoint to the bare `remix-i18next` and `entry.client.tsx` derives its hydration namespaces from the bundled resources (`getInitialNamespaces` is dropped in v8). The nested `vite` 7.x toolchain copy disappears because `@react-router/dev@8` drops its `vite-node` dependency and widens its vite peer to ^7||^8, so the dev toolchain uses the app's own vite 8.x and dev-only audit advisory 1120790 has nothing left to flag (closes #398). The React Router entry files also shed their stale Remix v2 scaffolding header comments (`entry.client.tsx` and `entry.server.tsx` referenced `npx remix reveal` and linked to remix.run docs for a framework this app no longer is), realigning with the current upstream default that scaffolds them comment-free (the functional `eslint-disable` on `entry.server.tsx` is retained). **Action required:** raise your local Node to >=22.22.0 (the v8 engine floor; `.nvmrc`/`.node-version` pin 22.23.1 on the 22.x LTS line) (#444, #450)
...
- `app/components/Header/`, `app/components/Footer/`, `app/components/GaiaLogo/`, and `app/assets/images/gaia-logo.svg` from the template: these GAIA-branded surfaces are owned by the manifest and removed from the scaffold directly rather than shipped and stripped at init. Adopters who customized any of these owned files will be offered a migration note by `/update-gaia`; the neutral index + `Layout` ship in their place. The maintainer logo the README still references was relocated (recovered from git history) to `docs/assets/gaia-logo.svg`, outside `app/` so it never enters the app build and under a release-excluded path so it is never bundled into the `create-gaia` distributable (#455, #458)
...
- harden `/gaia-init` (and its `/setup-gaia` successor) across real init runs. A single AskUserQuestion timeout no longer cascades into unattended auto-defaulting of every downstream identity gate: each gate is evaluated independently (no carried-forward absence inference), a fixed HARD-BLOCK tier (run mode, pnpm consent, language, i18n teardown, CODEOWNERS, title, slug, CI intent) re-asks rather than auto-answers while a SAFE-DEFAULT tier (Step 9 tool modes, mentorship) re-asks once then applies its default, and free-text identity values are never fabricated (an unknown CODEOWNERS handle ships a loud `REPLACE-WITH-YOUR-GITHUB-HANDLE` placeholder surfaced as a required follow-up). Init also stops claiming the release-excluded CI workflows ship (`code-review-audit.yml` and `forensics-triage.yml` are absent from the scrubbed tarball): it no longer git-mv's absent workflow files on Skip or prints a premature token/secret/label recipe on Yes, recording only the enable/decline intent and the local audit `default_mode` baseline. Real-run defects are fixed: `pnpm install` survives `--no-tty` (`--config.confirm-modules-purge=false`), spec-kit registration stages through an in-project `mktemp -d` instead of a sandbox-forbidden `/tmp` path (`setup-cloned-gaia-project` gets the same fix), the Serena verify probe anchors to `^serena:`, `strip-branding` de-brands the Storybook brand and removes the logo, and the invalid bare-handle `.github/CODEOWNERS` is dropped from the template. And react-doctor v0.5.8's changed install layout is handled end to end: the non-Claude `.agents/skills/react-doctor/` copy is stripped (parents `rmdir`'d without touching unrelated `.agents/` content), the install-verification probe repoints from `~/.claude/skills/react-doctor` to the gitignored `.claude/skills/react-doctor` where v0.5.8 installs the Claude Code skill, and `/setup-gaia`'s `pnpm pkg delete … scripts.react-doctor` uses the bracket-quote form that no longer throws `ERR_PNPM_UNEXPECTED_TOKEN_IN_PROPERTY_PATH` (#417, #439, #504, #509)
...
- `/update-deps` preview and override handling are correct: the emitted payload gains a per-group `snoozed[]` (`group`, `targets`, `snoozed_at`, `resurfaces_at`) reusing the existing suppression logic the CLI previously computed only as the aggregate `actionable_count` and discarded before emit, so the preview marks a snoozed group `[snoozed until DATE]` and default-skips it (with an explicit "update everything incl. snoozed" override) instead of silently re-offering a group snoozed at the exact offered version every run; and the override audit re-resolves with `pnpm dedupe` instead of `pnpm install` (which short-circuits "Already up to date" on an overrides-only change and could leave a security-floor override unapplied), asserting the lockfile `overrides` block matches `pnpm-workspace.yaml` before finishing (#388, #500)
- close a series of distribution-boundary and manifest-tracking gaps, mostly surfaced by `/health-audit`, so maintainer-only surfaces cannot leak into an adopter bundle and the boundary greps stop mis-triaging shipped files. The `workflow-denylist` leak-check gains a quote-anchored alternation (`\[[^\]]*['"]!`) so an inverted entry smuggled into an inline flow array (`paths: ["src/**", "!*.md"]`) is flagged like the block-list and `paths-ignore:` forms it already caught, the quote anchor preventing a stray `[!` in a run-step string from false-positiving. The release-scrub wikilink-to-excluded check derives its excluded-slug set at scan time from `.gaia/release-exclude` (bare-directory excludes such as `wiki/entities` / `wiki/meta` walk for the pages beneath them, matching case-insensitively and alias/anchor-aware) instead of a hand-maintained regex alternation that drifted from the manifest. Two shipped skill reference docs (`.claude/skills/gaia/references/harden.md`, `forensics/redaction.md`) that named maintainer source/test paths are wrapped in balanced `gaia:maintainer-only` scrub markers; `.claude/rules/repo-relative-paths.md`, shipped but untracked in `.gaia/manifest.json`, is regenerated as `owned` and allowlisted for its sanctioned placeholders; the maintainer-only bats suites (`.gaia/scripts/tests`, `.github/audit/tests`) are excluded from the tarball; the runtime-created `.claude/wiki-recompact-pending` sentinel is added to the runtime-deps markers so the bundle check stops false-flagging it; and the maintainer-paths detection copies are synchronized while `.gaia/manifest.json` is regenerated fresh against the exclusion list. All maintainer-only, and the checks are config-driven, so no CLI rebundle (#430, #434, #497, #499)
...
- the `gaia scaffold` generators handle real inputs: `scaffold route` resolves output paths from `cwd` (matching the component and service generators), emits a flat `<kebab>.ts` locale file instead of a `<PageName>/index.ts` subtree, names page folders `<Pascal>Page`, fails loudly when the locale barrel is missing, and gains a `--dry-run` flag that previews writes without touching disk; and `scaffold component --props` accepts comma-bearing prop types (`Record<string, unknown>`, multi-arg function props, tuples) by splitting only on depth-0 commas, so one `--props` value can mix plain and complex-typed entries instead of erroring out (#397, #411)
...
- clear named toolchain security advisories across the app and the maintainer CLI: bump `@babel/core` to 7.29.7 (GHSA-4x5r-pxfx-6jf8), `js-yaml` to 4.2.0 (GHSA-h67p-54hq-rp68), and `esbuild` to 0.28.1 (GHSA-g7r4-m6w7-qqqr), and pin the `.gaia/cli` `vite` devDep to 8.0.16 (GHSA-fx2h-pf6j-xcff); `js-yaml` is a runtime dependency bundled into the `gaia` CLI binary, which was rebuilt. In the same `.gaia/cli` workspace (a separate pnpm workspace `/update-deps` doesn't reach, so hand-applied), the `tsx` devDependency is bumped 4.21.0 → 4.22.4 to clear its transitive `esbuild` off the `GHSA-g7r4-m6w7-qqqr` floor: tsx 4.22 widens its esbuild range to `~0.28.0`, so the vulnerable `0.27.7` copy drops out and the lockfile resolves a single `esbuild@0.28.1`, resolving Dependabot alert #116 (#413, #453)
