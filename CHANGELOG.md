...
- `/setup-gaia` reuses a CI token the repo or org already has instead of always prompting for one. Before the token-type question it lists the secret names reachable by the repo at both repo scope (`actions/secrets`) and org scope (`actions/organization-secrets`, which `gh secret list --repo` does not surface), names only and never values. If `CLAUDE_CODE_OAUTH_TOKEN` or `ANTHROPIC_API_KEY` is already present (the former taking precedence), it skips the question and all out-of-band provisioning; the rendered workflows already wire both env vars, so an org-visible secret is consumed automatically. This is the common case for a repo inside an org that sets the token org-wide (#509)
...
- `/gaia-init` and `/setup-gaia` handle react-doctor v0.5.8's changed install layout. The installer now also drops a `.agents/skills/react-doctor/` copy of the skill for any non-Claude agent it detects (Copilot, Warp); both strip steps remove it (and `rmdir` the now-empty parents without touching unrelated `.agents/` content) so the operator no longer has to spot and delete it by hand. gaia-init's install-verification probe is repointed from `~/.claude/skills/react-doctor` to `.claude/skills/react-doctor`, where v0.5.8 installs the Claude Code skill: the home probe failed on a clean machine and halted init, passing only where a stale home copy lingered. `.claude/skills/react-doctor/` is gitignored so the project-local skill stays per-machine and never lands in an adopter's committed tree. Also fixes `/setup-gaia`'s `pnpm pkg delete … scripts.react-doctor`, whose bare hyphenated key threw `ERR_PNPM_UNEXPECTED_TOKEN_IN_PROPERTY_PATH` and aborted (deleting neither script), by adopting the bracket-quote form gaia-init already used (#509)
