...
- routine dependency refresh: `knip` 6.17.1 → 6.18.0, `lint-staged` 17.0.7 → 17.0.8, `isbot` 5.1.43 → 5.1.44, `nanoid` 5.1.14 → 5.1.15, and `@vitejs/plugin-react` 6.0.2 → 6.0.3. All in-range minor/patch bumps with no code or override changes; `vite` 8.0.16 → 8.1.1 stays held by the release-age cooldown (#463)
