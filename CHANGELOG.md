...
- a per-section dollar-cost line in every `tokens.md` the token tally writes: the `## Planning` and `## Execution` sections and the single spec doc now each carry an estimated-USD cost line, priced from that section's own per-model token usage as a generation-time snapshot placed beneath the token block, using the same committed rate table and effective-dated pricing as the merge roll-up. The rate-table resolution and per-model per-bucket arithmetic now live in one shared lib (`.gaia/scripts/token-pricing-lib.sh`) that both the tally and the roll-up source, and the roll-up's own rendered output is unchanged. Pricing never guesses a rate and never blocks: input it cannot price degrades to an `unavailable` marker, undercounted input to a marked `lower bound`, and the tally always exits 0 (#558)
...
- retires the per-folder `cost.md` the token tally wrote (no human read it) for a compact `cost.json` sidecar keyed by phase kind, and reroutes the fail-closed pre-delete cost gate to read it (an unparseable sidecar still blocks a delete, the SPEC-026 invariant intact; a legacy `cost.md` is honored as a transitional fallback). Reverses merged-SPEC retention: a merged SPEC folder's `SPEC.md`, `AUDIT.md`, and `cost.json` are now kept at merge and age-reaped by the SessionStart janitor only once merged past a retention window (`GAIA_SPEC_RETENTION_DAYS`, default 30) and cost-represented, while spec-less `PLAN-NNN` folders and colocated plan subfolders still delete at merge. The plan orchestrator reconciles its id-ledger the moment its PR merges (new `plan-reconcile.sh` for the plan arm, `spec-reconcile.sh` for the spec arm). Amends the unreleased delete-at-merge and per-folder `cost.md` behavior above; pre-release, so no adopter-migration burden. (SPEC-029) (#606)
...
- merged spec and plan working folders are now deleted at merge instead of archived, gated on the folder's cost first being fully represented in the durable `.gaia/local/telemetry/cost.jsonl` ledger (plus the id-ledgers), which becomes the sole record that survives; the shared ledger write behind this is now concurrency-safe under parallel worktrees (locked, with timeout-safe degradation so no row is ever lost). A separate, human-gated one-time migration is available to clear the pre-existing `archived/` backlog under the same cost-representation guarantee.
...
- GAIA's unreleased token-cost artifacts are consolidated and hardened: `tokens.md`/`tokens.jsonl` become `cost.md`/`cost.jsonl`, each feature's spec, planning, and execution cost merges into one dollars-led `cost.md` at archive time (with the archived spec folder flattened), and the durable ledger becomes a fresh, versioned, self-describing `cost.jsonl` data contract for an external cost dashboard, documented on a new `Cost Data Contract` wiki page with `token-tally.sh` as its single source of truth. Also adds a terminal `abandoned` spec status with a guarded empty-draft sweep, so a never-authored draft stops resurfacing for resume. Pre-release, so no adopter-migration burden. (#569)
