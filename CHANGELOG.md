...
- the `rm -rf` guard hook (`.claude/hooks/block-rm-rf.sh`) missed most of the ways a dangerous command is actually written, and denied only the sloppiest spelling of each. Four bypasses are closed. A *quoted* target (`rm -rf "$HOME"`) was allowed, because the guard word-split its targets but never stripped quote characters; the brace form (`${HOME}`) was allowed, because no pattern matched it; an operand written before the flags (`rm $HOME -rf`) was allowed, though GNU `rm` permutes its arguments and deletes home all the same, so the shape is live on Linux while looking harmless when hand-tested on macOS; and a dangerous target in any but the first `rm` of a chained command (`rm -rf dist && rm -rf /`) was allowed, because only the first segment was ever inspected. `--no-preserve-root` was also unreachable when written ahead of the recursive flag. The common thread is an inversion worth naming: the guard caught the careless spelling of a target and missed the careful one, so writing good shell made you *more* likely to slip past it. This is defense-in-depth rather than an open door, `settings.json` pre-approves only narrow scratch-path `rm` forms, so a bypassing command still met a normal permission prompt; the missing second layer is what is restored, and the hook's header now states plainly what a text-matching guard cannot see (computed targets, command substitution, `xargs`). The hook also gains the table-driven bats suite it never had, 50 deny/allow cases across every axis above, whose first run is what surfaced three of the four bypasses (#699)
