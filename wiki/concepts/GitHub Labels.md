...
**Label spellings.** A rename is complete when the old spelling reaches no consumer, and that half is **verified by search rather than enumerated**. Record the previous name in `.gaia/labels.json`'s `renamedFrom` as part of the rename itself: that entry is what `gaia labels sync` reads to issue `gh label edit <old> --name <new>`, and it is also the term a scan of the tracked tree searches for. So rename registry-first, then `git grep` the old spelling, and each namespace prefix the rename retires, until the tree is clean of both. A rename that never records `renamedFrom` is punished by `gaia labels sync` creating a second label instead of renaming the first.
...
`.gaia/scripts/debt-count-refresh.sh`, `.gaia/statusline/gaia-statusline.sh`, and `.claude/hooks/debt-session-reconcile.sh` are untouched by every **namespace** rename in the namespace paragraphs below, so they are named once here rather than per paragraph, as the **count/statusline/hook three**: `.gaia/scripts/debt-count-refresh.sh` excludes exactly three label names (`in-progress`, `debt:spec-pending`, and `debt:spec-active`) and ignores the rest, `.gaia/statusline/gaia-statusline.sh` parses no labels, and `.claude/hooks/debt-session-reconcile.sh` only reconciles the count downward. Parsing no labels is not the criterion: `.gaia/statusline/gaia-statusline.sh` parses none either and stays on the list. It is named here because the count/statusline/hook three is a group about namespace renames, not a subset of that list.

The scope is those paragraphs and not this whole section: the `in-progress`, `debt:spec-pending`, and `debt:spec-active` labels above are in the governed set too, and `.gaia/scripts/debt-count-refresh.sh` and `.gaia/scripts/check-debt-issue-metadata.sh` hardcode all three spellings, but only the two `debt:` park labels still carry a namespace to rename, so a namespace rename reaches just those two labels. `.claude/rules/issue-claim.md` and `.claude/hooks/issue-claim-release.sh` are untouched by a namespace rename for that same reason, since `in-progress` is the only spelling either one carries. They are deliberately not counted into the count/statusline/hook three, which is a group defined by parsing a namespace and finding nothing to change, not by being unaffected.
