...
Output: `.gaia/local/handoff/<tree_key>/HANDOFF-{YYYY-MM-DD}-{slug}.md` (the directory is gitignored); `<tree_key>` identifies the working tree, printed by `bash .gaia/scripts/main-root-lib.sh --tree-key`. The file is ephemeral: only one handoff exists at any time. Writing a new handoff first deletes any existing one (`rm -f .gaia/local/handoff/*/HANDOFF-*.md`); the new file carries forward anything unfinished from the previous session.
