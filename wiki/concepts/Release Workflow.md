...
- `.github/workflows/shell-lint.yml`: runs shellcheck over GAIA's framework bash via the `.gaia/tests/shell-lint.sh` harness. Adopters run that bash as GAIA-controlled code they never author, so the linter guarding it has no adopter surface. The harness also folds in five custom pattern lints beyond shellcheck, each for a class shellcheck cannot model: `lint-hook-array-guard.sh` flags a bare `"${arr[@]}"` over a possibly-empty array, which aborts a hook under `set -u` on the bash 3.2 macOS ships; `lint-git-path-quoting.sh` flags an executed `git diff --name-only` or `git ls-files` that omits `-z` (under `core.quotePath`, a path carrying a non-ASCII byte or control character prints C-quoted, silently breaking every downstream pattern match); `lint-workflow-run-interpolation.sh` flags a `${{ ... }}` expression sitting inside a workflow `run:` block body, since GitHub Actions substitutes it into the run script's text before bash parses it, so a value carrying a quote or `$(` is parsed as shell syntax rather than passed as data, while the `env:` form has no such hazard; `lint-grep-ere-escapes.sh` flags a backslash-escaped letter in an extended-regex `grep` pattern, which BSD grep on macOS and GNU grep on the runner resolve differently, so one pattern means two things; and `lint-errexit-status-read.sh` flags a read of `$?` following a bare command-substitution assignment while errexit is armed, since the assignment takes the substitution's status and the shell exits there, leaving the read and every branch it feeds unreachable. The last three reach the `run:` bodies of workflows and composite actions, which no `*.sh` glob sees.
