...
- `.github/workflows/shell-lint.yml`: runs shellcheck over GAIA's framework bash via the `.gaia/tests/shell-lint.sh` harness. Adopters run that bash as GAIA-controlled code they never author, so the linter guarding it has no adopter surface. The harness also parses every tracked `*.sh` with bash 3.2, the version stock macOS ships as `/bin/bash` and the version GAIA's own scripts declare support for, because shellcheck models bash 5's grammar and a construct that is a syntax error only on 3.2 clears every shellcheck pass; where no bash older than 4 exists (any Linux runner) that pass skips loudly rather than silently, and it fails rather than reporting clean when the interpreter is missing or unreadable. Because every Linux runner takes that skip, the workflow carries a second job on `macos-latest`, the one image in GAIA's CI whose `/bin/bash` is a real 3.2.57, and runs `shell-lint.sh --only bash32-parse` there: the pass-selection flag keeps that leg to the one pass whose verdict depends on the host's interpreter, since macOS runner minutes bill at 10x and every other pass duplicates the ubuntu leg. That leg asserts the image's `/bin/bash` major version before running, because inheriting the pass's exit-0 skip would leave it green having parsed nothing. It also folds in custom pattern lints beyond shellcheck, each for a class shellcheck cannot model: `lint-hook-array-guard.sh` flags a bare `"${arr[@]}"` over a possibly-empty array, which aborts a hook under `set -u` on the bash 3.2 macOS ships; `lint-errexit-source-guard.sh` computes the errexit-reachable source closure over `.claude/hooks/**` and `.gaia/scripts/**` and flags any load in that closure left unbracketed, since sourcing a present-but-unparseable file under errexit abandons the shell at the load, and in a `PreToolUse` hook that exit code is the deny code, turning an interrupted update or a truncated write into a gate that refuses every matching call, including the edit that would repair the library; `lint-git-path-quoting.sh` flags an executed `git diff --name-only` or `git ls-files` that omits `-z` (under `core.quotePath`, a path carrying a non-ASCII byte or control character prints C-quoted, silently breaking every downstream pattern match); `lint-workflow-run-interpolation.sh` flags a `${{ ... }}` expression sitting inside a workflow `run:` block body, since GitHub Actions substitutes it into the run script's text before bash parses it, so a value carrying a quote or `$(` is parsed as shell syntax rather than passed as data, while the `env:` form has no such hazard; `lint-grep-ere-escapes.sh` flags a backslash-escaped letter in an extended-regex `grep` pattern, which BSD grep on macOS and GNU grep on the runner resolve differently, so one pattern means two things; `lint-errexit-status-read.sh` flags a read of `$?` following a bare command-substitution assignment while errexit is armed, since the assignment takes the substitution's status and the shell exits there, leaving the read and every branch it feeds unreachable; and `lint-oracle-blind-invocations.sh` flags a script invocation, anywhere in the source directories the capability oracle's closure walks, that the oracle's anchors do not match, by differencing bash's own reading of command position against the anchors' reading of the same line, since a shape the anchors reject leaves the capability manifests silently incomplete about what that file reaches for. The path-quoting, run-interpolation, ERE-escape and errexit status-read guards reach the `run:` bodies of workflows and composite actions, which no `*.sh` glob sees; the oracle-blind guard does not, reading only the `*.sh` files under the scan roots it declares.
