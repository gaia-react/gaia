...
- `.github/workflows/shell-lint.yml`: runs shellcheck over GAIA's framework bash via the `.gaia/tests/shell-lint.sh` harness. Adopters run that bash as GAIA-controlled code they never author, so the linter guarding it has no adopter surface. The harness also folds in two custom pattern lints beyond shellcheck: `lint-git-path-quoting.sh` flags an executed `git diff --name-only` or `git ls-files` that omits `-z` (under `core.quotePath`, a path carrying a non-ASCII byte or control character prints C-quoted, silently breaking every downstream pattern match), and `lint-workflow-run-interpolation.sh` flags a `${{ ... }}` expression sitting inside a workflow `run:` block body, since GitHub Actions substitutes it into the run script's text before bash parses it, so a value carrying a quote or `$(` is parsed as shell syntax rather than passed as data; the `env:` form has no such hazard.
