...
- **No names** → spawn `code-audit-frontend`, fail-closed: never treat an empty or unanswerable result as "nothing owed". An in-scope file no member owns owes `code-audit-frontend` the same way (see [[Code Audit Team#Ownership classifier]]), so nothing in scope escapes silently.
