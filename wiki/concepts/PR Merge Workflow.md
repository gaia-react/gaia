...
| Every changed file is out of audit scope (bypass)                         | `pr-merge-audit-check.sh`    | The PR's full diff against its merge base with the branch it merges into (read from the pull request itself, so a PR stacked on a non-default branch is judged on its own changes rather than on its base branch's history; an unresolvable base widens back to the repository default) touches only out-of-scope surfaces: `wiki/`, `.claude/`, `.specify/`, `.gaia/`, `docs/`, root-level markdown. The agent has no rules that apply, so no marker is required. This mirrors `code-review-audit.yml`'s `has_source` skip locally, so the gate clears even when the installed workflow predates the out-of-scope status stamp or CI is absent. Evaluated fail-closed: any in-scope path (`app/`, `test/`, configs, `.github/workflows/`) keeps the marker mandatory, so a PR carrying auditable source can never reach this bypass. |
...
The PreToolUse hook `post-findings-block-on-merge.sh` posts one consolidated findings block to the PR on every `gh pr merge` invocation whose resolved audit mode is `local`, deterministically, no hand-run step required. It resolves the shared pull-request-wide base through the resolver's argument-less form, the same base every dispatched member reaches for keying its own artifacts, and calls the existing producer. A member's own per-member review base narrows what that member actually reviewed, but every member still keys its findings sidecar to this one shared base, which is what lets one key serve the whole dispatched set: a member keying its sidecar to a base of its own would leave its findings out of the block with no error raised anywhere. `.gaia/scripts/check-audit-base-derivation.sh` is the standing check on that (see [[Code Review Audit Agent#Incremental scope]]). The rendered payload carries a `review_bases` entry for each member whose sidecar records one: that member's own per-member review base, the reason it anchored there, and the clearance tree that anchored it, so a reviewer sees each member's own scope decision alongside the merged findings.
