...
- `resolved_mode == local`, or the workflow absent → **run the local agent** as the producer; on a clean pass it writes the marker, stamps the trailer (pushing the stamp commit only when the stamp created one), then posts the `GAIA-Audit` success status on the pushed head so the github.com button clears too.
...
`--path <tempfile>` is accepted alongside these and stays useful when a member's text *is* in hand (the completion notification carries it): it adds the return-text arm described below. It is not required, and fabricating an empty file to satisfy it classifies no-op.
...
The chore(deps) bypass mirrors the same skip narrowing that `code-review-audit.yml`, `tests.yml`, and `chromatic.yml` apply at CI level. All four surfaces (local hook + three required workflows) release together when a `chore(deps):` or `chore(deps-dev):` PR is recognized, so dep-bump PRs from `/update-deps` are turnkey. The local path's stamp and status hooks (`audit-stamp-trailer.sh`, `post-audit-status.sh`) read the same predicate against the pull request's title, so on a dep-bump diff that also dispatches a specialized member, that member's own earned marker completes the handshake without a default-member run; both waive the missing default-member marker only, never a default-member refusal, and fail closed when the title cannot be read. The bypass requires `gh` to be installed and authenticated; if either is missing the hook falls through to the normal deny path (the bypass is opt-in proof, not a fallback).
...
1. Add the entry to the right `### Added | Changed | Removed | Fixed` subsection under `## [Unreleased]`, present tense with a trailing `(#<issue>)` reference naming the issue the change closes, or `(#<PR>)` when the change closes no issue. An issue names the problem an adopter recognizes where a pull request names the diff, and a pull-request number is the one reference that always exists, so the fallback keeps the rule total. Leave the reference on an already-released entry alone: a released block records what shipped. Write it at Keep a Changelog altitude: 1-3 sentences on what changed and why it matters, not implementation mechanics (no file/function/flag-internals narration). Preserve any **Action required:** marker and its literal command, breaking/migration substance plus a pointer to the steps, behavior-changing flag names, adopter-relevant version/engine bumps, and a truthful who/why clause; deep detail belongs in the PR and commit.
