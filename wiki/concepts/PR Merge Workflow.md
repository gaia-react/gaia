...
**The named root binds the handshake, not just the review.** A member scopes every clearance write, trailer stamp, status POST, and sidecar path to the root the dispatch named, passing it as `--root` to `audit-write-clearance.sh`, `audit-stamp-trailer.sh`, and `post-audit-status.sh`. Only when the dispatch names no root does the ambient checkout stand in. The distinction is invisible in a plain feature-branch session, where the two are the same directory, and load-bearing under worktree isolation, where the reviewed tree is a linked worktree while the member's own working directory is the main checkout: a root taken from the cwd there keys a marker to content the member never read, which the gate either fails to find or, worse, believes.
