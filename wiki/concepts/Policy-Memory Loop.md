...
3. **TTL tally.** A background refresher recomputes, each TTL, how many distinct PRs carry each valid `finding_class` across a rolling 90-day window, at any severity. It drops classes already promoted (a rule exists) or locally declined without fresh evidence, and writes a candidate count to the statusline cache. It separately reports the `holistic/unclassified` recurrence in a named `unclassified` field, tracked by the same distinct-PR/90-day window. It is never promoted into a candidate, and it is subject to a machine-local suppression keyed on the classless fallback. The tally is an ephemeral projection: nothing is staged, an un-acted pattern decays by ageing out of the window.
4. **Statusline nudge.** When the candidate count is above zero, the statusline shows a `Run /gaia-harden (N recurring patterns)` segment, alongside the `/update-deps` and `/update-gaia` indicators and under the same setup-complete suppression. A non-zero `unclassified` count raises the same one segment, and when both counts are above zero the two reasons stack inside it (`Run /gaia-harden (2 recurring patterns, 3 unclassified)`) rather than rendering a second segment: both are discharged by the same bare `/gaia-harden` run, so no command occupies more than one segment. The tally carries a `gh_ok` flag alongside `candidate_count`; on a `gh`/network outage it emits `candidate_count 0` with `gh_ok false`, and the statusline refresher keeps the previously cached count instead of treating the failure as a genuine all-clear.
5. **Judge the form.** `/gaia-harden review` judges the lowest-context-weight form that fits the finding (deterministic check, skill, or path-scoped prose rule) and whether to edit an existing artifact or author a new one, then drafts it into the working tree for the engineer to **approve**, **decline**, **defer**, or **redirect**. The `unclassified` signal is read separately, as a seed-a-class-or-investigate prompt, never offered through this approve/decline/defer/redirect loop.
   ...
   - The `unclassified` signal is not a draftable candidate, so the approve/decline/defer/redirect loop never reaches it. It carries a suppression instead, recorded against the classless fallback, and it persists until the window's distinct-pull-request count rises at least 3 above the recorded count. An age-out is a second way it clears: the prune drops a fallback-keyed entry on the same terms as any other, once the classless cluster stops recurring at the threshold, so the recorded baseline never outlives the findings that justified it.
