...
The two halves land in different files, and the boundary holds only because Claude Code resolves settings **per key** rather than per object. `gaia sandbox apply` writes its seed fragment to the gitignored `.claude/settings.local.json` (`.gaia/cli/src/sandbox/seed.ts` builds the fragment, `.gaia/cli/src/sandbox/apply.ts` merges it), and that fragment carries no `filesystem` key at all, while the deny list is committed to `.claude/settings.json`. The local file outranks the committed one, so a harness that resolved the whole `sandbox` object from the highest scope setting it would drop the committed deny list on exactly the machines that turned the sandbox on, silently: no error, and the sandbox still reports enabled.
...
So the seed carries no `filesystem` block of its own, deliberately. Re-stating the deny list there would put a second copy of the boundary in a per-machine file, and the merge helper replaces array values wholesale rather than element-wise, so that copy would also clobber whatever local arrays an adopter had set.
