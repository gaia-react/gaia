...
The two halves land in different files, and the boundary holds only because Claude Code resolves settings **per key** rather than per object. `gaia sandbox apply` writes its seed fragment to the gitignored `.claude/settings.local.json`, and that fragment carries no `filesystem` key at all, while the deny list is committed to `.claude/settings.json`. The local file outranks the committed one, so a harness that resolved the whole `sandbox` object from the highest scope setting it would drop the committed deny list on exactly the machines that turned the sandbox on, silently: no error, and the sandbox still reports enabled.

It resolves per key instead, and the harness documentation says so in two places. On merging: "When you define the same filesystem array in multiple settings scopes, Claude Code merges them, combining paths from every scope rather than replacing one scope's array with another's", and "For array keys such as `excludedCommands` and `allowRead`, Claude Code merges entries from every scope the session loads, so a developer can append entries that widen the policy" ([sandboxing docs](https://code.claude.com/docs/en/sandboxing.md)). On precedence: "When the same key appears in more than one place, Claude Code uses the value from the highest level that sets it" ([settings docs](https://code.claude.com/docs/en/settings.md)). Wholesale replacement of the `sandbox` object would make that documented filesystem-array merge impossible, so "the same key" is per key, and a scope setting no `filesystem` key sets nothing there. All three sentences read against Claude Code 2.1.274 (2026-09-17).

So the seed carries no `filesystem` block of its own, deliberately. Re-stating the deny list there would put a second copy of the boundary in a per-machine file, and `gaia sandbox apply` replaces array values wholesale rather than merging them element-wise, so that copy would also clobber whatever local arrays an adopter had set.

<!-- gaia:maintainer-only:start -->
GAIA maintainers: the fragment is built in `.gaia/cli/src/sandbox/seed.ts` (`SandboxSettingsFragment`, which names no `filesystem` key) and merged by `.gaia/cli/src/sandbox/apply.ts`, whose deep merge recurses into plain objects and assigns anything else, arrays included, straight through.
<!-- gaia:maintainer-only:end -->

To re-verify in a throwaway repo: put a `sandbox.filesystem.denyRead` entry for `canary.txt` in `.claude/settings.json`, the seed's own shape in `.claude/settings.local.json`, enable the sandbox, and run a sandboxed `cat canary.txt`. A refusal means the committed deny still reaches the subprocess tier through a local enable.
