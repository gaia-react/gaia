...
- **The `rate_table_id` recipe.** `sha256` of the committed `token-rates.json` file's raw bytes, truncated to the first 16 hex characters, prefixed `sha256:` (e.g. `sha256:1a2b3c4d5e6f7890`). When the machine-local overlay is pricing at least one model, the overlay file's bytes fold into the same hash, so a row priced under shipped-plus-overlay cannot claim the identity of the shipped table alone; the prefix and the 16-hex shape are the same either way. `.gaia/scripts/token-pricing-lib.sh`'s `gaia_hash16` and `gaia_rate_table_id` are the implementation anchor, and both `token-tally.sh` and `cost-reprice.sh` delegate to them so the writer and the re-pricer cannot compute the identity differently. This is the exact identity a downstream reader uses to re-price a stored `by_model` under a known rate card.
- **The machine-local rate overlay.** A model with no entry in the shipped table prices at zero, which is a lower bound rather than a cost. `.gaia/local/token-rates.local.json` supplies rates for such a model without touching the shipped file, which is manifest class `owned` and therefore either overwritten or turned into a conflict patch by the next `/update-gaia`. The overlay merges over the shipped table per model key: a key replaces that model's **whole** window array, because the array's order carries the effective-dated selection above, and `cache_multipliers` is shipped-only. It may override a model the shipped table already prices, so a wrong rate has the same local remedy as a missing one. Before merging, each model's value must be a non-empty array of window objects: an empty array, a non-array, or an array of bare numbers is refused outright rather than merged, since any of those shapes would otherwise announce the overlay as active while pricing the model at zero or raising an error inside `rate_window` at pricing time. An explicit `--rate-table` bypasses it entirely, keeping a fixture table hermetic. Anything unreadable about the overlay leaves pricing on the shipped table and says so on stderr, never on an empty table. `.gaia/scripts/cost-unpriced-scan.sh` reports which models are unpriced and which the overlay is covering, and the overlay is never registered in `.gaia/manifest.json`: being outside the manifest is what makes it survive an update.
