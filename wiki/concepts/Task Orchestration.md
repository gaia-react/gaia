...
Invoked via `/gaia-plan [description]`; see [[GAIA Plan]] for the skill surface. For implementation work involving multiple files or subsystems, Claude generates a plan + orchestrator structure so each piece of work runs in a fresh-context sub-agent and the orchestrator drives the whole thing end-to-end. A spec-less one-off plan's artifacts live under `.gaia/local/plans/{slug}/`; a plan derived from a SPEC colocates inside the SPEC's own folder at `.gaia/local/specs/<SPEC-ID>/plan/` (`plan-2`, `plan-3`, … for a revision).

## Plan artifacts

1. **Per-task docs** in the plan folder: self-contained for fresh-context sub-agents (context, dependencies, interface contracts, files to touch, acceptance criteria). `.gaia/local/plans/{slug}/` is home for a spec-less one-off plan; a spec-derived plan colocates at `.gaia/local/specs/<SPEC-ID>/plan/`.
...
5. **Archival.** After all implementation phases pass and the user confirms the PR is ready to merge, the orchestrator runs `.gaia/scripts/plan-archive.sh` on its own plan folder, which prunes everything except `SUMMARY.md` and `tokens.md`. A spec-less plan's pruned folder moves to `.gaia/local/plans/archived/<slug>/`. A spec-colocated plan is pruned in place inside `specs/<SPEC-ID>/plan/`; it rides into `specs/archived/<SPEC-ID>/plan/` when the SPEC folder is later archived. `.gaia/local/` is gitignored by the GAIA default (checked via `git check-ignore`), so the prune-and-move is invisible to git and no commit is needed. If the path is tracked, the orchestrator commits and pushes the move as the **final commit on the PR**. If the user explicitly asks to keep the folder un-archived, the orchestrator skips the step and reports.
