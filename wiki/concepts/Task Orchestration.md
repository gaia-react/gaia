...
5. **Archival.** After all implementation phases pass and the user confirms the PR is ready to merge, the orchestrator runs `.gaia/scripts/plan-archive.sh` on its own plan folder, which prunes everything except `SUMMARY.md` and `tokens.md`. A spec-less plan's pruned folder moves to `.gaia/local/plans/archived/PLAN-NNN/`. A spec-colocated plan is pruned in place inside `specs/<SPEC-ID>/plan/`; it rides into `specs/archived/<SPEC-ID>/plan/` when the SPEC folder is later archived. `.gaia/local/` is gitignored by the GAIA default (checked via `git check-ignore`), so the prune-and-move is invisible to git and no commit is needed. If the path is tracked, the orchestrator commits and pushes the move as the **final commit on the PR**. If the user explicitly asks to keep the folder un-archived, the orchestrator skips the step and reports.
