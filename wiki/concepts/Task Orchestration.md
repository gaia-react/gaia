...
`SUMMARY.md` is a runtime artifact (not authored at plan-write time): the orchestrator creates and appends to it as phases complete (see lifecycle step 2 below). `SUMMARY.md` and `cost.md` are the two files the archive step preserves; everything else in the plan folder is pruned away.
...
5. **Archival.** After all implementation phases pass and the user confirms the PR is ready to merge, the orchestrator runs `.gaia/scripts/plan-archive.sh` on its own plan folder, which prunes everything except `SUMMARY.md` and `cost.md`. A spec-less plan's pruned folder moves to `.gaia/local/plans/archived/PLAN-NNN/`, and `plan-archive.sh` appends a `## Total` so its `cost.md` carries `## Planning` + `## Execution` + `## Total`. A spec-colocated plan is pruned in place inside `specs/<SPEC-ID>/plan/`, no `## Total` yet, the consolidation below owns that. When the SPEC folder is later archived, a shared consolidate-and-flatten routine folds the SPEC-root `cost.md`'s `## SPEC` section together with the plan's `## Planning` + `## Execution` into one `## SPEC` + `## Planning` + `## Execution` + `## Total` document at the SPEC root, moves `SUMMARY.md` up beside it, and removes the now-empty `plan/` subfolder, so `specs/archived/<SPEC-ID>/` never nests a `plan/`. `.gaia/local/` is gitignored by the GAIA default (checked via `git check-ignore`), so the prune-and-move is invisible to git and no commit is needed. If the path is tracked, the orchestrator commits and pushes the move as the **final commit on the PR**. If the user explicitly asks to keep the folder un-archived, the orchestrator skips the step and reports.
