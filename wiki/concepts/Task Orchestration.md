...
5. **Deletion.** After all implementation phases pass and the user confirms the PR is ready to merge, the orchestrator runs `.gaia/scripts/plan-archive.sh` on its own plan folder, which now deletes it outright: a spec-less plan's `.gaia/local/plans/PLAN-NNN/` folder and a spec-colocated plan's `specs/<SPEC-ID>/plan[-N]/` subfolder are both removed rather than pruned-and-moved. `cost.jsonl` and the two id-ledgers are the whole durable record that survives; `SUMMARY.md` and `cost.md` are deleted with the folder. `.gaia/local/` is gitignored by the GAIA default (checked via `git check-ignore`), so the delete is invisible to git and needs no commit. If the user explicitly asks to keep the folder, the orchestrator skips the step and reports.
