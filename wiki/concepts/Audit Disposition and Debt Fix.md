...
The clear is compare-and-clear rather than an unconditional `rm`. `.gaia/local/debt/` is shared across every linked worktree, so a peer worktree's merge can arm the sentinel during the `gh` call that sits between the age probe and the clear. The refresher re-reads the mtime immediately before clearing and removes only the sentinel it actually decided about: one that is absent, re-armed, or newly armed since the probe reads as a mismatch and survives to be serviced by a later tick. Without that check the run would delete an invalidation nothing has answered, and the cached count would hold whatever this recompute wrote until the next sentinel event or the TTL.
