...
**`holistic/unclassified` is not a security trigger.** The closed vocabulary is small by design, so the fallback is the *expected* class for most out-of-scope findings, not a signal that a finding is unknown or dangerous. It means the finding sits outside the **telemetry** vocabulary and nothing more. Keying the security screen on it would divert every out-of-scope finding on a public repo, file nothing, and leave the debt backlog permanently empty and `/gaia-debt` unable to fix anything: an off switch rather than a gate. The security screen and the telemetry vocabulary are independent axes.
