...
- **ephemeral** — transient scratch, regenerable or discardable, self-pruned by its own producer or left for a human to clear. Keyed by session, spec id, or a uuid, or an idempotent-overwrite cache whose concurrent write is harmless. Needs no tree key and no preservation. *Examples:* in-flight SPEC drafts and gate caches; react-perf run output; the daily version-check lock.
...
One coupling follows from the single symlink and is worth an author's attention: **the write-guard does not see per-tree stale writes.** A checkout-boundary guard catches a stale pre-switch path writing one tree's state into another's only while per-tree entries are non-exempt. Once the whole directory legitimately resolves to main, the guard cannot tell a correct per-tree write from a stale one. The per-tree isolation boundary sits inside `.gaia/local`, at the tree key, where a checkout-boundary guard structurally cannot reach. Under this model **the key is the isolation.**
...
**What trips that test is `git` sharing a command with a command substitution, and the controls below separate it from what runs.** `AUDIT_ROOT="<literal path>"; git -C "$AUDIT_ROOT" rev-parse --show-toplevel` **runs**, so a variable in the `-C` operand is not itself the trigger, and neither is a compound that names `git`. `X="$(git rev-parse --show-toplevel)"; echo "$X"`, a nested `git` with no `-C` and no parameter-expansion default, is **refused**. So is `git -C <literal path> merge-base … > <file>; BASE="$(cat <file>)"`, whose substitution holds no `git` at all: the two only have to share the command. The token is matched as a substring, so a `.github/` path inside a substitution trips it too, while the same script invoked by its literal path as a plain command runs. A multi-command block trips the general test above however its `git` calls are spelled, so the repair is the one the message asks for: one plain command per call, each value a command prints typed into the next one as a literal.

**Two further tests refuse on the dispatched arm without naming a construct the refusal above does.** A command that feeds `git` its arguments from stdin (`xargs`, `parallel`) is refused because the repository it targets cannot be verified, and a command name computed at runtime inside a substitution (`D="$("$ROOT/.gaia/scripts/<script>.sh" …)"`) is refused because it cannot be shown not to be `git`. The `code-audit-*` member specs are written against all of them: every command that names `git` is plain, every root and value is typed literally, and the one derivation that needs a multi-command body, the review scope, runs as a single script invoked by its literal path (`.gaia/scripts/audit-resolve-scope.sh`), which the runtime does not look inside. The default member's disposition pipeline carries no exception: the eligibility set comes from that same scope-resolver call under `--eligibility`, which runs under dispatch confinement the same as everything else on this list.
