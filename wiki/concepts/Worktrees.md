...
**Two further tests refuse on the dispatched arm without naming a construct the refusal above does.** A command that feeds `git` its arguments from stdin (`xargs`, `parallel`) is refused because the repository it targets cannot be verified, and a command name computed at runtime inside a substitution (`D="$("$ROOT/.gaia/scripts/<script>.sh" …)"`) is refused because it cannot be shown not to be `git`. The `code-audit-*` member specs are written against all of them: every command is plain, every root and value is typed literally, and the one derivation that needs a multi-command body, the review scope, runs as a single script invoked by its literal path (`.gaia/scripts/audit-resolve-scope.sh`), which the runtime does not look inside.
