...
5. **Hand off.** Print a short summary of the plan, then a fenced one-line resume prompt (`Read /abs/path/to/.gaia/local/plans/{slug}/KICKOFF.md and execute it.`). The skill prints the prompt as a fenced code block, then a single trailing `Type /clear and paste the prompt above.` line.
