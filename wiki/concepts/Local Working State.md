...
- **Orphaned audit markers.** Two key families share this drop zone, so the sweep keeps each by its own rule. A **tree**-keyed clearance marker (`audit/<tree>.ok`, and the per-member `audit/<tree>.<member>.ok`) gates `gh pr merge` only while its `<tree>` is the tree being merged, so the janitor keeps it when the key is HEAD's tree, or the tree of any local branch tip or any worktree's HEAD (a still-open feature line, or a parallel audit in another checkout). A **commit**-keyed `audit/<sha>.dispositions.json` is kept when its `<sha>` is HEAD or reachable from a local branch. Everything else is removed, including a key that resolves to no git object at all. Once a PR squash-merges, the audited tree is no longer the tree of any live branch and the marker is spent. The tree key is what makes the Code Audit Team's members order-independent (see [[PR Merge Workflow]], Marker key), and it is why this sweep cannot simply ask whether a key is a valid commit: a tree key never is.
