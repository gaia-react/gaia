...
| `audit/<tree>.ok`, `audit/<tree>.<member>.ok` (earned) | [[Code Review Audit Agent]] merge gate | ephemeral | spent once its tree is no longer live; additionally survives `GAIA_AUDIT_MARKER_RETENTION_HOURS` (default 72) hours past that point, so it can still serve as a carry-forward anchor |
| `audit/<tree>.carried`, `audit/<tree>.<member>.carried` (carried); `audit/<tree>.refused`, `audit/<tree>.<member>.refused` (refused) | [[Code Review Audit Agent]] merge gate | ephemeral | spent once its tree is no longer live; no retention-window extension |
| `audit/<sha>.dispositions.json` | [[Code Review Audit Agent]] merge gate | ephemeral | spent once orphaned; retained alongside its marker when the marker survives by the retention window above |
...
- **Orphaned audit markers.** Two key families share this drop zone, so the sweep keeps each by its own rule. A **tree**-keyed clearance, earned (`audit/<tree>.ok`, per-member `audit/<tree>.<member>.ok`), carried (`audit/<tree>.carried`, per-member `.carried`), or refused (`audit/<tree>.refused`, per-member `.refused`), gates `gh pr merge` only while its `<tree>` is the tree being merged, so the janitor keeps it when the key is HEAD's tree, or the tree of any local branch tip or any worktree's HEAD (a still-open feature line, or a parallel audit in another checkout). An **earned** marker gets one more chance beyond reachability: it also survives for `GAIA_AUDIT_MARKER_RETENTION_HOURS` (default 72) hours past its own recorded audit time, so it can still serve as a [[PR Merge Workflow#Carry-forward|carry-forward]] anchor after the session that earned it ends; its disposition sidecar is retained alongside it for the same window. A carried clearance or a refusal gets no such window, reachability is the only thing keeping either alive. A **commit**-keyed `audit/<sha>.dispositions.json` is kept when its `<sha>` is HEAD or reachable from a local branch, or when the earned marker it belongs to is kept by the retention window above. Everything else is removed, including a key that resolves to no git object at all. Once a PR squash-merges, the audited tree is no longer the tree of any live branch and the marker is spent (unless the retention window still covers an earned one). The tree key is what makes the Code Audit Team's members order-independent (see [[PR Merge Workflow]], Marker key), and it is why this sweep cannot simply ask whether a key is a valid commit: a tree key never is.
