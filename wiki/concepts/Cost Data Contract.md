...
| `unpriced` | `array` of model id, omitted when empty | Every `claude-` model in `by_model` that the pricing rate table had no window for, so `dollars` omits that model's share and is a lower bound. Present only when at least one model went unpriced, which keeps its absence the ordinary case rather than a signal. A reader finds affected rows by this field instead of re-deriving which keys the table was missing at write time; `.gaia/scripts/cost-reprice.sh` re-prices them once the table carries the model. See [[Token Cost Readout]]. |
