...
`.gaia/local/telemetry/cost.jsonl` is a versioned, self-describing cost ledger: one JSON record per `/gaia-spec` run, `/gaia-plan` run, KICKOFF-execution run, or maintenance-command run (the `--command` closed set in `.gaia/scripts/token-tally.sh`), durable enough for an external dashboard to read directly. `.gaia/scripts/token-tally.sh` is the single source of truth for the emitted schema, this page documents what it builds, not the other way around; when the two disagree, the script wins.
...
| `rate_table_id` | `"sha256:<16-hex>" \| null` | Identity of the rate card that priced `dollars`, so a downstream reader can re-price the raw `by_model` under a different card. The committed table's bytes alone, or those bytes plus the machine-local overlay's when the overlay is pricing at least one model, so two rows priced under materially different cards never share an id. `null` off the priced path. See [[Token Cost Readout]] for how a rate card earns this id. Absent (not present, not `null`) on `source: "backfill"` rows. |
| `unpriced` | `array` of model id, omitted when empty | Every `claude-` model in `by_model` that the pricing rate card had no window for, so `dollars` omits that model's share and is a lower bound. Present only when at least one model went unpriced, which keeps its absence the ordinary case rather than a signal. A reader finds affected rows by this field instead of re-deriving which keys the card was missing at write time; `.gaia/scripts/cost-reprice.sh` re-prices them once the card carries the model. The field is a **historical record, not a live verdict**: it says what was missing when the row was written, so a consumer asking what this machine cannot price *today* recomputes from `by_model` rather than reading it, which is what `.gaia/scripts/cost-unpriced-scan.sh` does. See [[Token Cost Readout]]. |
...
## Concurrent writers, and the one window that stays open

Every writer serializes on one shared cost mutex keyed on the main checkout's telemetry directory, so parallel worktree sessions append to the single ledger without losing a row. Two writers matter: `token-tally.sh` appends, and `cost-reprice.sh` rewrites the whole file.

The mutex is not unconditional, and the exception is deliberate. `token-tally.sh` runs from hooks, so it holds a **never block a hook** contract: when it cannot acquire the mutex within its timeout it degrades to an **unlocked** append rather than waiting or skipping. That append is invisible to the mutex, and therefore visible to a concurrent whole-ledger rewrite.

`cost-reprice.sh` carries the other half. It runs classify, backup, and replace as one critical section inside the mutex, and immediately before its atomic replace it re-reads the ledger's tail and carries through any row that arrived after its classify pass, unpriced and byte-identical. A ledger that shrank mid-run refuses the replace instead of guessing, because the tail arithmetic only holds while the first N lines are still the N that classify read.

**This narrows the loss window; it does not close it.** A row appended between that re-read and the rename is still lost. Closing it outright would require the degraded append to write somewhere the rewrite cannot reach, which costs a second on-disk shape plus a fold-in in every reader of the ledger, and the exposure does not earn that: it needs a manual re-price to overlap a lock starvation, and what is lost is one row of advisory cost telemetry. The `.bak` copy does not hold such a row either, since the backup predates it.

The rule the two scripts split between them: **never block the hook** is kept on the appending side, **never lose a row** is kept on the rewriting side, where a row can actually be observed.
