...
| `rate_table_id` | `"sha256:<16-hex>" \| null` | Identity of the rate card that priced `dollars`, so a downstream reader can re-price the raw `by_model` under a different card. The committed table's bytes alone, so two rows priced under materially different cards never share an id. `null` off the priced path. See [[Token Cost Readout]] for how a rate card earns this id. Absent (not present, not `null`) on `source: "backfill"` rows. |
| `unpriced` | `array` of model id, omitted when empty | Every `claude-` model in `by_model` that the pricing rate card had no window for, so `dollars` omits that model's share and is a lower bound. Present only when at least one model went unpriced, which keeps its absence the ordinary case rather than a signal. A reader finds affected rows by this field instead of re-deriving which keys the card was missing at write time. The field is a **historical record, not a live verdict**: it says what was missing when the row was written, so a consumer asking what this machine cannot price *today* recomputes from `by_model` rather than reading it. See [[Token Cost Readout]]. |
...
A merged folder's archival unit is its consolidated `SUMMARY.md` plus its `cost.json` sidecar, kept at merge; the `/gaia-spec` pre-flight sweep age-reaps the folder once its merge has aged past the retention window (`GAIA_SPEC_RETENTION_DAYS`, default 30 days) and its cost is fully represented in this ledger, a check the delete/reap gate (`cost-represented.sh`) drives from the `cost.json` sidecar itself, cross-checked against this ledger. A spec-less `PLAN-NNN` folder takes the symmetric path: kept at merge, reduced to `SUMMARY.md` + `cost.json`, but has no automatic age-reap; it is deleted only by hand-running `plan-close`, which drives the same fail-closed gate, keyed on `plan_id` instead of `spec_id`. A spec-colocated plan's `plan[-N]` subfolder is deleted once its own `PROGRESS.md` has fed the parent SPEC's `SUMMARY.md`; the parent SPEC folder remains the archival unit. Either way, nothing kept on disk carries the cost forward beyond the sidecar: `cost.jsonl` (this ledger) plus the two id-ledgers (`specs/ledger.json`, `plans/ledger.json`) are the whole durable record. Every figure a reaped folder's `cost.json` sidecar once carried, per-phase buckets, totals, and dollars, is recoverable from this ledger's rows keyed by `spec_id` or `plan_id`, with no dependency on any sidecar file surviving.
...
Every writer serializes on one shared cost mutex keyed on the main checkout's telemetry directory, so parallel worktree sessions append to the single ledger without losing a row.
