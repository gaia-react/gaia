...
Separate from the three streams above, `gaia ping` (`src/ping/`) sends a fire-and-forget POST to `https://telemetry.gaiareact.com/ping` when `/gaia-init`, `/setup-gaia`, and `/update-gaia` complete. The body carries the event name (`init`, `setup`, or `update`), the per-install `projectId` (the deterministic id at `.gaia/local/.project-id`, shared with the mentorship/cloud streams), the GAIA version, the OS platform, and a handful of low-cardinality categorical fields specific to the event (e.g. `mode`/`i18n`/`ci` for `init`; `type`/`mentorship`/`repo`/`ci`/`audit` for `setup`; `from`/`to` for `update`). `GAIA_TELEMETRY_PING_DISABLE=1` suppresses it; there is no other opt-out. The stable `projectId` makes the pixel pseudonymous-per-install rather than fully anonymous, correlating one install's `init` -> `setup` -> `update` events, it never carries user paths or free text. This ping is distinct from the mentorship/cloud/analytics streams documented above; a network failure, timeout, or unreadable manifest never affects the caller's exit code.
