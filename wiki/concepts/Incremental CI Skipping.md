...
the diff covers the full PR scope. That branch is `GITHUB_BASE_REF`, which
Actions sets on every `pull_request` event, so a PR stacked on a branch other
than `main` falls back to its own base rather than to the repository default,
whose divergence it never introduced; `origin/main` is the answer whenever no
base ref is declared. The helpers never anchor on an un-passed commit, so they
