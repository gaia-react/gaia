...
`.gaia/scripts/check-audit-base-derivation.sh` holds the arrangement in place. Over `.claude/agents/`, no review base may come from a bare `merge-base` against the default branch, the named `FULL_BASE` self-skip derivation being the one exemption, and every definition naming `BASE_SHA` must also name the resolver.
