...
`.gaia/scripts/check-audit-base-derivation.sh` holds the arrangement in place with three assertions over `.claude/agents/`. No review base may come from a bare `merge-base` against the default branch; the named `FULL_BASE` exemption, covering both a specialist's self-skip derivation and the default member's eligibility derivation, is the one exception, by name, because the name is what tells a legitimate whole-PR fork point apart from a drifted review base. Every definition naming `BASE_SHA` must also name the resolver. And no `diff --name-only` may consume the base without a three-dot range, which is what keeps the reviewed scope and the digest on the same content.
