...
### The terminal action, stated once

Detection and the single hardened re-dispatch are the same on every surface. What follows a **second** consecutive no-op is that surface's terminal action, and this page owns the general rule: **inline fallback wherever the caller may legitimately do the work itself**, applying the recovered result exactly as if the subagent had returned it and recording the unit as degraded rather than clean or empty. Each surface still states its own **preamble**, what to restore, snapshot, or re-clear before the one retry, because that part is genuinely site-specific; only the ending has a single owner.

**One surface departs from that ending, on purpose.** The pre-merge audit gate ([[PR Merge Workflow]]) stops and surfaces the double no-op to the operator instead of falling back inline, because a Code Audit Team member's clearance marker is that member's own attestation: an orchestrator that reviewed the diff itself inline would be writing a clearance nobody earned. Stopping is safe there in a way it is not elsewhere, because that gate is fail-closed, no marker still means no merge. That page states the departure and its reason; a surface not named there takes the inline ending above.

### Ad-hoc dispatches: `--shape agent-report-file`

`audit-noop-detect.sh` also exposes a general-purpose `--shape agent-report-file` mode for any ad-hoc subagent dispatch outside those three surfaces: the dispatcher pre-clears an output path (`rm -f`), has the agent write its result there instead of returning it inline, then classifies the file the same way, real vs. no-op. The shape is real when the file exists, parses, and its report array resolves, the top-level value, or the value of `--report-key <key>` when the caller passes one. A caller that knows its own denominator asserts it with `--expect-count <n>` or `--min-count <n>` (mutually exclusive), because a truncated write parses fine and otherwise reads as a real result. An empty array with no count assertion is real: "the agent wrote an empty answer" is a result, "the agent wrote nothing" is not. Its ending is the general one above, one hardened re-dispatch against the re-cleared path, then inline fallback; the retry prefix itself is written out verbatim in `.claude/agents/code-audit-frontend.md` ("No-op detection and retry for each refuter"). `.claude/rules/subagent-dispatch.md` codifies when a dispatching agent should use this pattern rather than asking for a result in the reply.
...
**Keys stay shared and are deliberately never per member.** The value keying a member's findings sidecar and the shared re-run ledger is one pull-request-wide base every dispatched member reaches through the resolver's argument-less form. `gaia_audit_key` (`.gaia/scripts/audit-key-lib.sh`) combines that one shared base with the branch. A member keyed to a different base for this purpose reads and writes a different shared re-run ledger from its co-dispatched siblings, with no error raised anywhere; a member's own per-member review base never keys anything.

The consolidated findings block is deliberately not one of the readers that key agreement protects. `post-findings-block.sh` selects on the branch half alone, across every base, because the shared base legitimately advances one stamp per cleared round: reading one key would post only the final round, which is clean by construction. The sidecars stay partitioned per round; only the read spans them.

Every member pipes its findings array into `audit-write-findings.sh --findings -` through a single-quoted `printf` payload rather than naming a shared file path. The payload is a `printf` rather than a heredoc because worktree isolation refuses a heredoc outright, which leaves that form unrunnable on any pull request audited from a linked worktree; the single quotes do the job the quoted delimiter used to, holding a `$` or a backtick inside a finding literal. The wave of members runs in parallel against one shared session scratchpad, so a literal placeholder path is the same filename every member would write to: one member's array can land on another's sidecar, and a stale file left by an earlier round can republish as a fresh report. Stdin keys the write to the invoking member's own process instead.
...
The ledger keys on the shared pull-request-wide base (`git merge-base "$BASE_REF" HEAD`, resolved the same way every dispatched member resolves the shared base through the resolver's argument-less form), not HEAD, and not any member's own narrower per-member review base. That base is the fork point only while nothing has cleared yet: the argument-less form returns the most recent ancestor of HEAD that passed a clean audit, so it equals the fork point on a branch's first round and moves to the newest clearance stamp after that. The per-member marker (`<digest>.ok` for the frontend, `<digest>.<member>.ok` for a specialist) and the dispositions sidecar (`<frontend-digest>.dispositions.json`) key on each member's own content digest, not HEAD, because they certify the content that member reviewed. A fix commit that touches frontend-owned content rotates the frontend digest, and both artifacts, just as a HEAD move used to; a commit that touches nothing any member owns and no machinery leaves every digest, and every marker, valid. The ledger accumulates "what is still wrong relative to the cleared base," and that base holds still across the HEAD moves each fix commit produces, so the remaining items survive them with no HEAD-chaining logic. It holds still within a round, not across rounds: a round that clears stamps a trailer, and the next round's base is that stamp, so each round writes its own ledger and every member co-dispatched in one round shares it. Its `remaining[]` carries in-scope open findings only; out-of-scope findings stay in the dispositions sidecar (see [[Audit Disposition and Debt Fix]]).
