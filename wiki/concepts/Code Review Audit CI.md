...
The `Check for source-code changes` step diffs only the **un-audited delta**: `<audit-base>...HEAD`, where `<audit-base>` is the shared pull-request-wide base, resolved by `.github/audit/resolve-audit-base.sh`'s argument-less form: the most recent ancestor that already passed a clean whole-team audit under the current `.gaia/VERSION`. The default member's own review, dispatched inside the same job, separately resolves its own **per-member** review base through the resolver's `--member` form, and can anchor later than this shared floor when its own earned clearance covers more of the range; the member's actual reviewed delta can therefore be narrower than what this gate step decided was worth running for at all. When the shared delta touches no audit-relevant files (TS/TSX/CSS under `app/`, tests, `.storybook`, workflow files, or root-level build/lint/test configs), the gate short-circuits and reports `code-review-audit` green. Because the shared base is the last clean-audited commit, a PR that audits clean and then adds a prose-only commit (wiki, CHANGELOG, instruction files) reviews an empty auditable delta and skips; it does not re-run on a tree whose code was already cleared. With no audited ancestor the base is the branch the PR merges into (`GITHUB_BASE_REF`, `origin/main` when none is declared), so the delta is the full PR diff and first-audit behavior is unchanged.
...
A genuinely-clean audit that produces no self-heal commits and no empty trailer commit leaves the `push-fixes` step with neither `pushed` nor `marker_only` set. Without intervention, HEAD carries no `GAIA-Audit` status and the merge hook blocks the merge even though the audit passed. The same gap opens when the audit is clean but its only tree delta is a **refused** self-heal (see [[#Self-heal staging guards]] for the refusal reasons). `refused` means the fix was off-limits to auto-apply, or the step itself could not complete cleanly, not that the audit failed or the tree is dirty, so it does not gate this stamp either.
