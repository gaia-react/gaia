...
1. **Get description + GH-mirror opt-in.** Use `$ARGUMENTS` if non-empty; otherwise ask "What do you want to spec?" and wait. Then ask the GitHub-issue preference via `AskUserQuestion` (recommended/default: "No, skip GitHub issue"), persisting the answer as `gh_mirror_optin`. The mirror is off by default; this single signal gates step 10.
