...
11. **`/gaia-plan` handoff.** No `on_save` hook exists in spec-kit, so the handoff lives inline at the end of the wrapper. After the canonical save, `/gaia-spec` prints a copy-pasteable `/gaia-plan` handoff prompt naming the repo-absolute `SPEC.md` path (and the sibling `AUDIT.md` when the audit produced one), then stops; the human runs `/gaia-plan` in a fresh session. See [[Task Orchestration#Topology]].
