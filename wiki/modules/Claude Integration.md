...
- `block-eslint-config-edit.sh`: refuses every edit to `eslint.config.*` at any path, on the filename alone. Fix the source, not the config. The bluntness is deliberate: an added preset spread is both the sanctioned `...lint.reactRouter` migration and a way to turn rules off, so no test over the edit separates them, and the message carries what the match cannot by naming the legitimate edit and how to make it.
...
`permissions.allow` covers routine git / gh / pnpm operations plus scoped edits for `.claude/**`, `.gaia/**`, `wiki/**`, and `CHANGELOG.md`. `permissions.deny` covers `.env` reads and writes (`Read(.env)`, `Edit(.env)`), `pnpm-lock.yaml` writes, `.husky/_/**` internals, force-push variants on `main`/`master`, and `git reset --hard HEAD~*`. `Edit(<glob>)` is the write-side rule form: the file permission checks match only `Read(path)` and `Edit(path)` rules, and an `Edit` rule covers every file-editing tool (Write, Edit, MultiEdit, NotebookEdit), so a `Write(<glob>)` rule is accepted, never matched, and draws a startup warning naming the settings file it sits in. `Read(.env)` already enforces against the Read tool and the Bash readers Claude Code recognizes (`cat`, `head`, `tail`, `sed`) at any depth; `block-env-read.sh` delivers the residual coverage that literal match misses, the `.env.<env>` variant family and the Bash read paths outside that recognized set, as heuristic defense-in-depth, not a sandbox. The OS-level sandbox (`sandbox.filesystem` deny-read, merged with the Read/Edit deny rules) is the airtight opt-in for adopters who want enforcement that reaches arbitrary subprocesses; GAIA does not enable it by default. Both permission lists are alphabetized; path globs are repo-relative (no leading `/`).
