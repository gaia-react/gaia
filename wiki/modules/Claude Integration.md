...
- `block-eslint-config-edit.sh`: prevents modifying `eslint.config.*` to fix lint errors. Fix the source, not the config. An allowlist rather than a filename match: an edit passes only when every line it changes is blank, a comment, or a bare `...<name>.<group>` preset spread, and only when no spread is removed.
