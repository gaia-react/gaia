...
- `block-vitest-globals-tsconfig.sh`: prevents adding `vitest/globals` to any `tsconfig*.json`, reading all three payload shapes its matcher covers (`Edit`, `Write`, and `MultiEdit`'s `edits[]`). Use explicit imports.
