...
- `block-secrets-write.sh`: detects AWS keys, GH PATs, PEM headers, and dotenv-style assignments to `_TOKEN` / `_SECRET` / `_KEY` / `_PASSWORD` names that start the line; allows placeholders. It matches known shapes, not entropy.
