...
- `block-vitest-globals-tsconfig.sh`: prevents adding `vitest/globals` to any path whose name carries the substring `tsconfig.json`, reading both `Edit`'s `new_string` and `Write`'s `content`. Use explicit imports.
