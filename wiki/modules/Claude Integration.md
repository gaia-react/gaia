...
- `block-eslint-config-edit.sh`: prevents modifying `eslint.config.*` to fix lint errors. Fix the source, not the config. An allowlist rather than a filename match: comments and blank lines change freely and a bare `...<name>.<group>` preset spread may be added, while everything else, including reordering two spreads, is refused.
