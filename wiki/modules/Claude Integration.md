...
- `block-vitest-globals-tsconfig.sh`: prevents adding `vitest/globals` to any `tsconfig` file with a `.json` suffix, dot- or dash-separated, reading all three payload shapes its matcher covers (`Edit`, `Write`, and `MultiEdit`'s `edits[]`). Deliberately generous on the name, since `tsc` reads whatever config it is pointed at. Use explicit imports.
