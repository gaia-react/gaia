...
In a `#`-comment file the paragraph separator is a bare `#` line, and a block placed between two of them strips to both, so the adopter copy carries a doubled separator where a block it cannot see used to be. **The block owns the separator that introduces it**: the start marker comes first and the bare `#` sits on the next line, inside the markers. Exactly one separator survives, and deleting the block leaves the surrounding prose correctly separated, which is what the strip does. A block already sitting mid-paragraph or mid-list introduces nothing and needs no separator of its own. `.gaia/tests/distribution/03-marker-strip.sh` asserts it against the staged tree: no staged `#`-marker file may carry a doubled bare `#` separator, whether the strip produced it or the header authored it, since an adopter reads the same artifact either way. Each failure reports the source file's own count alongside the staged one, so a reader can tell how much of the doubling the header already carried: the strip and the header can each contribute one to the same file, and a report of `src=1 staged=2` is both at once.
