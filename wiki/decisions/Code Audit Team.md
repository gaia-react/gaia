...
<!-- gaia:maintainer-only:start -->
On this repo, an advisory, deliberately-not-required, release-excluded `Audit Roster Drift` workflow also runs the check on pull requests behind a paths-filter, giving the maintainer team an earlier signal than a local run alone would.
<!-- gaia:maintainer-only:end -->
