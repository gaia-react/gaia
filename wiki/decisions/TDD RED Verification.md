...
- **Any title-interpolating modifier** (`each` and `for`, across `test`, `it` and `describe`, including chained forms like `test.concurrent.each` and the tagged-template spelling), regardless of whether the declared title argument is itself a plain string literal. Vitest expands the row into the title at runtime (`$prop` or printf substitution), so for an interpolating title the declared argument is not what gets recorded. The extractor suppresses the whole modifier rather than predicting which titles interpolate: a substitution-free title such as `test.each(rows)('fixed title', ...)` is recorded verbatim and would in fact be matchable, so it is **exempted rather than unmatchable**, and that is a deliberate loss of coverage. Predicting the expansion means reimplementing vitest's substitution grammar, and erring permissive there restores the permanent block this carve-out exists to prevent.
