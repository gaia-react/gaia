...
- **The fold is a delete, and delete is not silent.** An upstream deletion of a file the adopter still has asks the adopter before removing it, and never auto-deletes (Update Workflow deletion table). Folding therefore manufactures a one-time per-file prompt on every adopter's next update. It adds an adopter-facing event; it does not remove one.
