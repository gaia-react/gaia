...
What is true after that repair, measured a file at a time: the group's heaviest shard is roughly 455 seconds on CI against the 780-second cap, and the binding leg is whichever scripts shard holds `check-script-capabilities.bats`, not either of the two legs named above. A fourth scripts shard would now lower it, where the earlier sizing found it bought nothing, and that option is foreclosed by the leg ceiling rather than by the floor: shards plus aggregator is already exactly what the poller window allows at two hops. So the lever that remains for this group is the same one that worked here, a suite's own runtime, and the next candidate for it is the leg now doing the binding.
