...
- [[Token Cost Readout]]: per-action token-to-dollar pricing off one shared pricing lib; the `by_model` field, the committed `token-rates.json` rate table, the roll-up's read-time dollar block, and the tally's own per-phase `tokens.md` snapshot cost line, each with its degrade markers.
