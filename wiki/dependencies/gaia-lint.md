...
Because `.gaia/cli` pins `@gaia-react/lint` independently in its own lockfile, nothing else keeps its version in step with the root workspace's pin. `.gaia/cli/src/lint-pin-parity.test.ts` asserts both workspaces declare the same version, and `cli-tests.yml` includes root `package.json` in its path filter so the guard runs on the pull request that causes any drift, not just on a later unrelated `.gaia/cli/**` change. The same test holds the rest of what the two workspaces must agree on in parity: the declared version of every devDependency both manifests share, the resolved version of every rule-bearing package both lockfiles freeze, and the supply-chain settings on each workspace. The test is the owner of what is compared and what is exempt (`MANIFEST_PARITY_EXEMPT` and `RULE_PACKAGE_PATTERN`), and a manifest assertion is on the declared version alone, never on behaviour. See [[pnpm]] for the full cross-workspace hardening.
