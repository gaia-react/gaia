...
marker=".gaia/local/audit/${TREE_SHA}.code-audit-maintainer-node.ok"
if [ ! -f "$marker" ]; then
...
```bash
.claude/hooks/post-audit-status.sh ".gaia/local/audit/${TREE_SHA}.code-audit-maintainer-node.ok"
```
