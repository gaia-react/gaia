...
Path: `$AUDIT_ROOT/.gaia/local/audit/${base}.code-audit-maintainer-node.findings.json`, the **same** `base` you already resolve at the start of every run (see "Remit and self-skip" above), never a second base resolution. If `base` is empty (resolution failed), skip the sidecar write entirely.
