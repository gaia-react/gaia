...
marker=".gaia/local/audit/${HEAD_SHA}.code-audit-maintainer-shell.ok"
if [ ! -f "$marker" ]; then
...
```bash
.claude/hooks/post-audit-status.sh ".gaia/local/audit/${HEAD_SHA}.code-audit-maintainer-shell.ok"
```
