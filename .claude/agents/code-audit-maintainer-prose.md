...
A narrower `changed` shifts one risk onto you: it can begin after a commit this PR already cleared, so a file that depends on the prose in front of you may be absent from the delta. Instruction files address one another by path and by section title, and nothing resolves those references until an agent is already midway through following one. When a changed skill renames a heading, renumbers a step, splits a reference file, or drops a target outright, `git grep` its path and the old heading across every tree your remit block names, plus `wiki/`, and read every file that points at it. Take the trees from that block rather than from a list written here: this sweep has to reach wherever executable prose lives, and a hand-kept list of roots goes stale the next time the remit gains one, silently narrowing the sweep while still reading as complete. This is one of your own dimensions seen from the other side: a pointer into a section that no longer exists still reads as a complete instruction, so the reader omits the step instead of stopping to ask.
