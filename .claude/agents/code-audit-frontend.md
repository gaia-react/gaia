...
```bash
bash <root>/.gaia/scripts/debt-origin-lib.sh --changed '<DEBT_ORIGIN_CHANGED>' --dir <root>
```

`<DEBT_ORIGIN_CHANGED>` is the finding's verdict, resolved once per finding under "Provenance `changed` field" (Rules-Based Audit, "Resolve the review scope"). The call prints one line. Carry that printed line as a literal onto its own line in the issue body, immediately after the `gaia-debt-key` line, never merged into it. If the call prints nothing or fails, omit the line and continue: **never block, fail, retry, or defer a filing because provenance is partial, absent, or malformed.** Provenance is diagnostic, not identity, so it is not a marker precondition, it never enters the disposition gate (section G), and the disposition-ledger sidecar (section F) gains **no field** for it. The field list, the value vocabulary, and the convention table live in `.claude/skills/file-tech-debt/SKILL.md`'s provenance section; this agent restates neither.
