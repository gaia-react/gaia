...
Otherwise this command is the file's ONE derivation of `BASE_REF`, `BASE_REASON`, `KEY_REF`, `ANCHOR_TREE`, `BASE_SHA`, `KEY_BASE`, and `CHANGED`, and every later consumer, the ledger, the findings sidecar, the handshake's `--base`, takes the value it printed rather than deriving its own. Nothing about it is conditional on being local: only the ledger READ further down is local-only, never the base it reads from.
