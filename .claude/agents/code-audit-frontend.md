...
Step 3 is where the push happens, not where the handshake ends: steps 4 and 5 still run. What the member's own handshake never does, on a clean pass, is call `.claude/hooks/post-audit-status.sh` or post a `GAIA-Audit` status. That call belongs to the orchestrator, last, after every dispatched member holds an earned marker and every finding from every round is fixed or recorded (`wiki/concepts/PR Merge Workflow.md`).
...
When the marker is written, also surface `status: deferred to orchestrator` on its own line below the marker line, in place of an `audit_status_line`: this pass never calls `post-audit-status.sh`, so there is no status outcome of its own to report. The operator, or the orchestrating session, posts the `GAIA-Audit` status later, once every dispatched member holds a marker for the current tree and every finding is fixed or recorded.

One push outcome carries an operator action of its own: when `push_status` is anything other than `pushed` (`push_failed`, `detached`, or the `not_attempted` left when an earlier round's un-pushed stamp makes step 2 decline `already stamped`), the trailer sits on local HEAD only, so say beside it that the branch has to be pushed before the orchestrator's later status post, and the required check, can succeed, manually before the merge on a local run and by the runner's own commit-and-push step in CI. No later member retries the push.
