...
The member's own handshake ends after step 3; it never calls `.claude/hooks/post-audit-status.sh` and posts no `GAIA-Audit` status on a clean pass. That call belongs to the orchestrator, last, after every dispatched member holds an earned marker and every finding from every round is fixed or recorded (`wiki/concepts/PR Merge Workflow.md`).
