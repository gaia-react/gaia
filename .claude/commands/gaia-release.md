...
`verify-required-checks.sh` checks the live branch-protection ruleset's required contexts against the workflows that actually run (it replaces the deleted per-PR `verify-required-checks.yml` lane).
...
The release PR is **not** exempt from the merge gate, `.claude/hooks/pr-merge-audit-check.sh` denies `gh pr merge` until every dispatched Code Audit Team member has a marker for the release commit's tree. Resolve the spawn set with the oracle (`bash .gaia/scripts/resolve-audit-spawn.sh`), then run the four-step protocol in `wiki/concepts/PR Merge Workflow.md` for each named member: spawn it on the branch, fix every Critical and Important finding, push (the tree changes), re-spawn the pending members on the new HEAD until every one writes its marker (see `wiki/concepts/PR Merge Workflow.md` `#### Marker key` for the marker filename, keyed to the member's content digest, not the tree sha). Knip / react-doctor advisories never block the marker.

The fix step for `code-audit-frontend` reads the re-run carry-forward ledger (`.gaia/local/audit/<audit-key>.rerun.json`, `<audit-key>` the incremental base sha plus the acting tree's own branch, `.gaia/scripts/audit-key-lib.sh`) for the deterministic list of what to fix (`remaining[]`) and what the last round already fixed (`fixed_last_round[]`), instead of relying on the previous round's full report. Every member re-spawned in the same round reads the same ledger, because neither half of the key moves with HEAD, and the key stays distinct from a sibling worktree's ledger even when both share a base sha. A round that clears stamps a trailer and the next round keys on that stamp, so each round that stamps has its own ledger rather than one file accumulating across the loop; a round that clears without stamping (an already-pushed attached HEAD) advances no key and reuses the previous round's ledger. The audit's LOCAL Task return is terse (pointer + counts); the per-finding detail is in the ledger. Fail-open: if the ledger is absent, corrupt, or stale, fix from the report's open findings as today. A specialized member carries no such ledger; its report carries its own findings directly.

Members write their markers only; they do not post the `GAIA-Audit` success status. Once `wiki/concepts/PR Merge Workflow.md` `#### Posting the status last` conditions hold, post it yourself, `bash .claude/hooks/post-audit-status.sh <path to a current member marker>`, before Step 11's merge; otherwise `--auto` queues against a required check nothing ever posts.
