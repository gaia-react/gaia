...
Members write their markers only, they no longer post the `GAIA-Audit` success status. Once every dispatched member's marker is in hand and every finding from every round is fixed or recorded under `## Accepted residuals (recorded, not fixed)` in the PR body, post it yourself, `bash .claude/hooks/post-audit-status.sh <path to a current member marker>` (contract: `wiki/concepts/PR Merge Workflow.md` step 3), before Step 11's merge; otherwise `--auto` queues against a required check nothing ever posts.
