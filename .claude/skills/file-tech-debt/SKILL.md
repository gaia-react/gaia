...
`.gaia/labels.json` is the last entry rather than a consumer at all: it is where every spelling this section governs is defined, so a rename is a registry edit by construction. Rename there by changing the entry's `name` and appending the old spelling to its `renamedFrom`, then regenerate the wiki page with `gaia labels docs`. `gaia labels sync` reads that file and nothing else, so a rename that works every consumer below and skips the registry leaves sync creating the old label forever and the new one never.
