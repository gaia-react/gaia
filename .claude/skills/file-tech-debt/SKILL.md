...
- A suggested fix.
- **The research block, on a `severity:investigate` filing only** (step 6). Three lines, byte-for-byte in this shape, anywhere in the body:
...
- **The queue is capped**, at three open `severity:investigate` issues (`INVESTIGATE_CAP` in `.gaia/scripts/check-debt-issue-metadata.sh`). Over the cap a filing is refused, and the two ways out are both the outcome the axis wants: resolve one of the open investigations, or grade this finding yourself, because the budget for not deciding is spent. Step 4 runs the check.
...
`severity:investigate` is the one value in the `severity:` namespace with a consumer outside the vocabulary set. `.gaia/scripts/check-debt-issue-metadata.sh` matches the bare spelling twice more, to decide whether the research block is required and whether the cap check reads the network at all, and `.claude/skills/gaia/references/debt.md` matches it in the ordering query's rank arm and in the fix-time exclusion. Both of those extra matches fail **open** on a forgotten rename, the same direction the claim label does: the block stops being demanded and the cap stops being counted, on a grade that still files. Rename the value and those four sites together. The body marker `<!-- gaia-investigate: v1 -->` is a second contract of its own, shared between the gate's patterns and step 5's schema, and it moves under the key-format rules above rather than these.
