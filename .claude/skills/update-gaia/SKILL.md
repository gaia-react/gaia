...
10. **Confined writes.** When `regions.regen.confined` is non-empty, name each entry with its action. `restored` and `removed` are writes the regeneration made inside the snapshot (7d), outside the region's declared paths, that the runner undid. `reported` is an entry the runner surfaced and deliberately left alone, and it has three origins, of which only the first lies outside the region's own directories. A write **outside the snapshot** was never a candidate for reverting at all. A path **inside it with no pre-image** to put back is an unreadable file, link, or directory, or a node holding nothing to read, such as a FIFO, socket, or device node; that last kind is not a write and reports on every run, including those the regeneration never touched. A path **inside it the runner declined to write** is one where the write might not have landed where the key names, where the adopter's own file could not be told apart from something the regeneration created, or where the revert was attempted and failed. A `reported` entry is the one the adopter has to look at, and a wholly new untracked directory surfaces as the directory itself rather than as its individual files.
