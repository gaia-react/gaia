...
- **Parse defensively.** There is no manifest validation on the adopter side; this flow reads raw JSON and iterates the file map. A `regions` key that is absent or an empty list means the same thing: zero declarations, no oracle call, no regeneration, and every file classified by the unmodified whole-file comparison exactly as it is without region awareness. A key that is **present but not an array** loads zero declarations too, and is **not** the same thing: it is a manifest this flow could not read, so Step 7d's runner refuses it by name (`refused[]`, `kind: 'manifest'`) and Step 9 renders that refusal ahead of the no-declarations row. Reporting the two alike would tell the adopter the release carries no regions when every region guarantee has silently not applied.
...
type RegenRegionsReport = {
  ...
  failed: Array<{argv: string[]; kind: 'exit' | 'killed' | 'spawn'; message: string; regionId: string; signal?: string; status?: number}>;
...
Every bucket Step 9 prints a command for carries its own `argv`, because the region id alone cannot be turned back into a command. `refused` declares `argv` optional because a `kind: 'declaration'` refusal has none; Step 9 owns what to print in its place. A `kind: 'manifest'` refusal is about the manifest itself rather than any one region, so it likewise has no `argv` and carries the sentinel `regionId` `(manifest)`; the one that occurs is a `regions` key that is present but not an array, which is a manifest the runner could not read and is deliberately not reported as the absent-`regions` case (Step 9 renders it as its own summary row, ahead of the no-declarations row).

`failed[]` separates two outcomes that both arrive without an exit status: `kind: 'spawn'` is an interpreter that never launched, and `kind: 'killed'` is a program that ran and was cut short by a signal (`signal` names it, e.g. `SIGTERM` on the runner's own runtime bound). Do not describe a `killed` region to the adopter as one that never ran; its declared paths may hold half-written output.
...
- **Writes are confined and backed up.** The runner writes nothing outside a region's declared path set: a write inside the region's own directories is reverted to what it held before the run, and a write anywhere else in the tree, which has no pre-image to restore from, is reported instead. A **deletion** inside those directories is a write like any other, so a file the regeneration command removes outside its declared paths is restored from the same pre-image; only the region's own declared paths may be deleted and stay deleted. It also copies every declared path it is about to rewrite into `$BACKUP_DIR` first, unless the merge walk already backed that path up.
...
2. **Failed or refused regenerations.** One entry per region, carrying the **literal argument vector** to run by hand, rendered from that entry's `argv` (for the region GAIA ships today that renders as `bash .gaia/scripts/write-audit-remits.sh`). Report each `failed[].kind` distinctly, because each has a different remedy; 7d's shape block above is the authority on the set, so read it rather than assuming these are the only three. `spawn` means the interpreter itself could not be launched. `exit` means the program ran and refused. `killed` means the program ran and was cut short by a signal (`signal` names it), which is the one case where the region's declared paths may hold half-written output, so never describe a `killed` region as one that never ran. Report per region rather than per path, because one command covers every path in the region, and the shipped writer aggregates per-member failures into a single non-zero exit after rewriting the members that did succeed.

   **A `kind: 'declaration'` or `kind: 'manifest'` refusal carries no `argv`, and that is correct.** A declaration too malformed to name an interpreter and an operand has no command to hand over, and a `manifest` refusal is about the manifest itself rather than any one region, so it has neither a command nor a region (it carries the sentinel `regionId` `(manifest)` and belongs in the summary row, not the per-region list). Print the defect, and state plainly that this release's declaration is unusable and there is nothing for the adopter to run: the remedy is upstream, not in their tree. Never fabricate a command for either.
