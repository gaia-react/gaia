...
10. **Confined writes.** When `regions.regen.confined` is non-empty, name each entry with its action. `restored` and `removed` are writes the regeneration made inside the snapshot, outside the region's declared paths, that the runner undid. `reported` is an entry the runner surfaced and deliberately left alone, either because there was no pre-image to put back or because it could not establish that writing the path was safe. Two things a `reported` entry is **not**. It is not necessarily outside the region's own directories: an in-scope path reports for either of those reasons, and an entry outside them was never a candidate for reverting in the first place. And it is not necessarily a write: an in-scope node the snapshot cannot read, a FIFO, socket, or device node, is recorded present with no pre-image and takes a `reported` row on every run, including runs where the regeneration never touched it. A `reported` entry is the one the adopter has to look at, and a wholly new untracked directory surfaces as the directory itself rather than as its individual files.
