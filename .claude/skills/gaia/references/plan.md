...
3.  **`{PLAN_DIR}/ORCHESTRATOR.md`**: instructions for running the plan. Must cover:
    ...
    - **Pre-flight isolation.** The generated `ORCHESTRATOR.md` carries this pointer, verbatim, as its pre-flight step:
      ...
      **Which tool writes the ledger depends on `RESOLVED_MODE`.** Under `feature-branch` isolation `{PLAN_DIR}` sits inside the acting checkout, so the ordinary `Edit`/`Write` tools write it and nothing below applies. Under `worktree` mode they cannot reach it: the harness isolates the session to the worktree and refuses an `Edit`/`Write` whose `file_path` resolves to the shared checkout, and both spellings of the path land on that same refused target, because a linked worktree reaches `.gaia/local` through one symlink to the main checkout. The refusal is the harness's, not GAIA's. `.claude/hooks/block-worktree-path-mismatch.sh` exempts the main-checkout plan and SPEC ledgers by design (it carries the rationale), so the exemption is real and editing that hook does not lift the refusal sitting above it. Write the ledger with `Bash` at its main-checkout absolute path instead, then read the file back and confirm both its content and its location before continuing. The read-back is what makes this fallback safe rather than a dodge, since the failure the discipline exists to prevent is a write landing in the wrong tree. Keep the fallback scoped to these main-anchored ledgers: a `Bash` redirect is never the way to write into another checkout where the edit tools already work.
