...
1.  **One task doc per parallel workstream**: `{PLAN_DIR}/task-{name}.md`. Each must be fully self-contained for a fresh-context sub-agent and include:
    ...
    - Acceptance criteria (concrete and testable). **When the task's deliverable is a guard, refusal, or gate, one criterion must drive it into its failing state and assert it refuses**, not merely observe it passing: passing is the state an inert guard is permanently in, so a criterion that only watches it pass cannot tell a working guard from a shipped-inert one. This binds a verification task the same way it binds the task that builds the guard (`.claude/rules/guards-must-fail.md`).
