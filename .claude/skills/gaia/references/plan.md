...
3.  **`{PLAN_DIR}/ORCHESTRATOR.md`**: instructions for running the plan. Must cover:
    ...
    - **Pre-merge Code Audit Team audit (roster-first, non-skippable).** Before any `gh pr merge` call, resolve which Code Audit Team members this branch's diff dispatches:
      ...
      On a clean pass each member writes its own marker and stamps the trailer, then stops there, its report reads `status: deferred to orchestrator` rather than posting the `GAIA-Audit` success status itself. The merge deny-hook requires **every** dispatched member's marker, so one member withholding holds the gate shut for all. If a member declines to write its marker, its report names what remains unaddressed; resolve those, commit, push (HEAD moves), then re-spawn the pending members on the new HEAD. A member that cleared the previous HEAD must be re-spawned too: its marker is keyed to the old SHA. Never hand-write a marker to bypass the gate. Once every marker is in hand and every finding across every round is fixed or recorded under `## Accepted residuals (recorded, not fixed)` in the PR body, and after this branch's last push, the orchestrator posts the success status itself, `bash .claude/hooks/post-audit-status.sh <path to a current member marker>`, before calling `gh pr merge` (contract: `wiki/concepts/PR Merge Workflow.md` step 3).
