...
- **Emit provenance on every filing.** Run `origin="$(bash .gaia/scripts/debt-origin-lib.sh --changed unknown 2>/dev/null || true)"` once per finding, then place `$origin` on its own line in the issue body, immediately after the `gaia-debt-key` line, never merged into it. The bare form is correct here and must not be "fixed" into the `cd "$AUDIT_ROOT" && ...` shape the agent's continuous-integration path uses: this route carries no `AUDIT_ROOT`, and the knowledge audit's own session cwd is the tree the filing describes. `changed` is the literal `unknown`, and that is correct rather than a degradation: this route holds no reviewed pull-request diff, so it has no fork-point changed-file set to test the cited path against, and it derives no base of its own to manufacture one. The field vocabulary and the convention table live in `.claude/skills/file-tech-debt/SKILL.md`; reference it, restate neither here. Fail open: if the helper prints nothing, omit the `gaia-debt-origin` line and continue, matching this section's existing posture that a backend-absent or transient `gh` failure is never fatal. Known limitation: on this route `branch`, `mode`, and `unit` describe the session doing the filing rather than necessarily the work that surfaced the finding, still honest since the fields report what the disposing agent observed, but not review attribution.
