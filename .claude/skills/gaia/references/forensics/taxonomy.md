...
| wiki-sync | `/wiki-sync` | "wiki-sync", "sync", "wiki commit" | `wiki/.state.json`, `wiki/log.md` last entry |
...
| dirty working tree blocks the workflow (e.g. `/wiki-sync` refused to push) | user-config | git state, fixable by user |
