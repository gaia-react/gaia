...
Four endings look like that controlled stop and are not it. Each keeps its claim, because the work may still be going somewhere, and `## Cost record (run end)` covers when each one's record is written. The arm list below states what each of them reports, except the three-round session cap, which has no arm of its own:
