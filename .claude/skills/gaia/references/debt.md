...
This reconcile queries and strips only `in-progress`. `debt:spec-pending` is a distinct, durable label parking a handed-off spec-class issue (see `## Fix-time spec screen`); this reconcile never iterates it and never strips it, spared by construction.
...
carries `in-progress`, or already carries `debt:spec-pending`) → state
...
- **Zero remaining candidates** (every open `tech-debt` issue already carries `in-progress` or `debt:spec-pending`) → do not prompt. State that every open `tech-debt` issue is already in progress or parked pending a SPEC, and stop. (Run ends here; see `## Cost record (run end)`.)
...
1. **Re-read each member's labels** (`gh issue view <n> --json labels`) before claiming. If `in-progress` is already present, a peer session won the race:
   ...
   If `debt:spec-pending` is present instead, the member is parked pending a SPEC handoff, mirroring the `in-progress` branch above exactly:
...
Run the ordering command above, then the clustering pass and the staleness probe, and print the backlog in sorted order: per issue, the number, title, severity band, age, cluster membership when it has any (e.g. `[batches with #B #C: same file app/foo/index.ts]`), `[in progress]` when the issue carries `in-progress`, `[needs spec]` when its emitted `handler` field is `"spec"` and it does not carry `debt:spec-pending`, `[spec pending]` when it carries `debt:spec-pending`, `[stale: path gone]` when the staleness probe found its dedup-key `path=` absent from the index, `[fold: <value>]` (e.g. `[fold: required]`) from the emitted `fold` field when it is non-null, and `[difficulty: <grade>]` (e.g. `[difficulty: medium]`) from the emitted `difficulty` field when it is non-null; an issue whose `difficulty` is `null` gets no difficulty annotation at all, and an issue whose `fold` is `null` gets no fold annotation at all, which is the ordinary case. The two spec annotations key on distinct signals, the emitted `handler` field versus the label, so an un-drained spec-class issue is never conflated with a handed-off one. `list` shows every open issue, including in-progress and spec-pending ones: it does not exclude them and it does not reconcile stale claims. Author nothing and prompt for nothing.
