...
3. **Rewrite every commit message on the branch that closes it**, `git commit --amend` for the tip commit and an interactive rewrite for an older one, then force-push. Step 4's rule means there is normally nothing to rewrite; read `git log --format=%B "refs/remotes/origin/<default>..HEAD"` and check anyway, where `<default>` is the repository's default branch (`git symbolic-ref --quiet refs/remotes/origin/HEAD` with its `refs/remotes/origin/` prefix stripped, `main` when that is unset) and the ref is spelled in full so a local branch or tag named `origin/<default>` cannot shadow it and hide commits from the range, because a surviving trailer reaches the squash-merge message and closes the issue as `COMPLETED` with nothing fixed, and no other step in this playbook reads commit bodies.
