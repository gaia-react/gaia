...
**When every candidate is excluded as investigate.** `fix` says so and names those issues with their open questions, rather than reporting an empty backlog. An investigate issue is resolved by answering its `gaia-investigate` block's question and then re-grading it (`gh issue edit <n> --remove-label severity:investigate --add-label severity:<tier>`, removing the now-answered block from the body in the same edit), which returns it to the pool as an ordinary candidate. `/gaia-debt` does not do that research itself: it has one terminal state, a merged fix pull request, and research ends in a re-grade.
