...
**When every candidate is excluded as investigate.** `fix` says so and names those issues with their open questions, rather than reporting an empty backlog. An investigate issue is resolved by answering its `gaia-investigate` block's question and then re-grading it, which returns it to the pool as an ordinary candidate. The re-grade is two edits, not one, because the label and the block live in different places and leaving either behind is its own defect: `gh issue edit <n> --remove-label severity:investigate --add-label severity:<tier>` swaps the grade, and a second `gh issue edit <n> --body-file <path>` replaces the body with one carrying the answer in its failure-mode prose and no `gaia-investigate` block. A swap alone strands the block, so the issue keeps asserting a question that has been answered and the gate reports `stray-investigate-block` on its next read. `/gaia-debt` does not do that research itself: it has one terminal state, a merged fix pull request, and research ends in a re-grade.
