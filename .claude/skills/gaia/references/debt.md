...
- **Every issue also carries `body`, `labels`, `key`, and `handler`.** `labels` is the label name list; `body` is the raw issue body, populated unconditionally; `key` is the parsed dedup key (`class`, `path`, an integer `line`) or `null` when absent or malformed; `handler` is the handler class taken from the `handler:<class>` label with its namespace prefix stripped (`prompt` | `plan` | `spec`), or `null` when the issue carries no `handler:` label. The value arrives with its `handler:` prefix stripped, so the field holds the bare class a reader prints. These ride the sort but play no part in it; the passes below read them.
