...
Resolve the main-anchored SPEC folder first, before any dispatch below consumes it. The SPEC folder is main-anchored state (state registry `specs-main`), so the report lands beside the artifact the ledger row indexes rather than in a second specs tree inside a linked worktree. The `mkdir -p` keeps this self-sufficient: step 3 creates the same folder from another file under another agent, and this step writes into it either way. `AUDIT.md` lands there per the tool-choice contract in Operational primitives.
