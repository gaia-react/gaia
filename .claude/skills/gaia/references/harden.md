...
- The `unclassified` signal is never a draftable candidate and is never auto-drafted: it carries no approve/decline/defer/redirect action and authors no artifact. It carries a suppression instead, on the terms `## Unclassified recurrence signal (seed-a-class-or-investigate)` states, including the prune exemption that leaves a recorded baseline governing whatever cluster arrives next. Relaxing this guardrail is deliberate: seeding provably cannot shrink the cluster it answers, so without a suppression a maintainer who does exactly the right thing is nagged for it for up to ninety days. No later change widens this further without the same justification.
