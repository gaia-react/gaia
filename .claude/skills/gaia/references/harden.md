...
- The `unclassified` signal is never a draftable candidate and is never auto-drafted: it carries no approve/decline/defer/redirect action and authors no artifact. It carries a suppression recorded against the classless fallback, and it re-surfaces once the window's distinct-pull-request count rises at least 3 above the count it was suppressed at. An age-out is not a second way it clears: the prune exempts a fallback-keyed entry on purpose, so the entry outlives the findings that justified it and the recorded baseline still governs whatever cluster arrives next. Relaxing this guardrail is deliberate: seeding provably cannot shrink the cluster it answers, so without a suppression a maintainer who does exactly the right thing is nagged for it for up to ninety days. No later change widens this further without the same justification.
