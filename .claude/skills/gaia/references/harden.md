...
- The `unclassified` signal is never a draftable candidate and is never auto-drafted: it carries no approve/decline/defer/redirect action and authors no artifact. It carries a suppression instead, on the terms `## Unclassified recurrence signal (seed-a-class-or-investigate)` states, and that suppression lives and dies with the evidence: the prune releases its baseline once the classless cluster stops recurring, so a recorded count never governs an unrelated cluster that arrives later. Relaxing this guardrail is deliberate: seeding provably cannot shrink the cluster it answers, so without a suppression a maintainer who does exactly the right thing is nagged for it for up to ninety days. No later change widens this further without the same justification.
