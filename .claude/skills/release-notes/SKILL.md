...
- Bullet strings are markdown-lite: inline code in backticks, links as `[text](url)` (rendered as a new-tab link). PR links (`https://github.com/gaia-react/gaia/pull/NNN`) are _optional_, include one only when a reader would genuinely follow it for detail, never as decoration. The exemplars below omit them.
