...
- Updates were confirmed. **Immediately bust the update-check cache** so the statusline reflects the post-update state on the next session regardless of whether this run completes. Use the Write tool to overwrite `.gaia/local/cache/shared/update-check.json`, preserving `gaiaCurrent`, `gaiaLatest`, `gaiaHasUpdate`, `serenaLangDrift`, and `auditDriftBaseline` from the existing cache (read it first), but setting `outdatedCount` to `0` and `checkedAt` to the current Unix timestamp. If `serenaLangDrift` or `auditDriftBaseline` is absent from the existing cache, omit it. The two are absent for different reasons and only one is self-correcting: `serenaLangDrift` is recomputed from source by the next refresher, while `auditDriftBaseline` is a *historical observation* (the size at which an over-budget file was first set aside because an open tech-debt issue covers it), so dropping it re-seeds the baseline at the file's current, larger size and silently disarms the growth check that re-fires the audit nudge. If the cache file does not exist, skip this step. (Snoozed groups are already excluded by the ledger on the next real check.)
