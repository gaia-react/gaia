...
For every override that was **retained** in Phase 0, in either workspace root, repeat the full Phase 0 toggle test (both the peer-dep and the security-floor check, re-capturing a fresh advisory baseline against the now-updated tree) now that surrounding packages have moved. A version that landed in Wave A or Wave B may have resolved the original peer-dep conflict or carried the patched transitive dependency that made a security-floor pin obsolete. The toggle test re-resolves with `pnpm dedupe`, never a bare `pnpm install`, exactly as in Phase 0. This is the last phase that mutates the `overrides:` map, so the lockfile settles here: close it with the same assertion Phase 0 runs, in the paragraph beginning **After the toggle loop, assert the lockfile matches config**, applied per root exactly as written there, repairing any drift with `pnpm dedupe` and re-running the quality gate. Follow that paragraph rather than a summary of it; one copy cannot drift against itself, and these two already have. Run this as a **Haiku agent**.
