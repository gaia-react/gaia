...
Adding a `.sh` under `.claude/hooks/**`, a `.md` under `.claude/rules/**`, or a hook registration in `.claude/settings.json` carries obligations no diff line shows. Whole-set checkers enforce most of them in CI, outside the Quality Gate, so run each one that applies before opening the pull request; the two tier bullets are judgment calls no check enforces. Rationale: `.claude/rules/file-placement.md` for hook state paths, [[Code Audit Team]] for tiers, [[Claude Hooks]] for jq and rooting.
