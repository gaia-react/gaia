...
`.gaia/manifest.json` is release-generated and lists only files GAIA ships. Feature work never adds to it; a path absent from the manifest is adopter-owned and invisible to `/update-gaia`, and its absence is not drift. A polluted manifest misleads the tools that read it — the `/gaia-fitness` drift check flags an adopter's own files as drift, and forensics capture ingests wrong data. The only legitimate writers are the release CLI, `/update-gaia`, the `remove-i18n` runbook (`.claude/instructions/remove-i18n.md`, dispatched from `/gaia-init`), and the `/gaia-fitness` `manifest` Fixer (the Bash writers use the `GAIA_MANIFEST_WRITE=` guard marker). See `wiki/concepts/Update Workflow.md`.
