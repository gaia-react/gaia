...
Before merging changes that touch any in-scope path, and before running `/wiki-sync` / `/wiki-lint` / `/wiki-consolidate`:
...
Any non-empty match outside this rule's prose is a candidate for rewrite. The narrative-vs-structural triage for the `.claude/`/`.specify/`/`.claude-tests/` greps is a human read — the regex flags candidates; the Exceptions section above codifies what stays.
