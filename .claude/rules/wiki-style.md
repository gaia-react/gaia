...
- **No inline PR / commit / date-of-change references in body prose.** Don't write "added in PR #97", "commit abc123 introduced …", "as of 2026-05-07 …". The git log answers those questions and stays accurate when prose drifts. This bullet is scoped to body prose; for code comments and bats test names, `.claude/rules/code-comments.md` decides whether a GitHub reference stays.
- **No conventions and no enumerations on a descriptive page, only a pointer to what owns them.** A page describing how something works does not restate the naming rule, the file list, the matcher set, the roster, the glob list, or the version. Name the rule file, the config file, or the command that holds it, and stop. This is the one rule the whole surface fails on. A blind multi-rater accuracy audit read every checkable claim on the wiki and the rules and found that not one defect was a page that had become conceptually wrong: every one was a cached fact the machine already answers. The pointer form cannot cache, which is the whole of why it is preferred. Corollary, which is where the reflex goes wrong: when an enumeration is found stale, **delete it and point**, do not complete it. A corrected list restarts the same decay from a fresher number, and the enumeration that was wrong twice is the one most likely to be wrong again.
...
- **GitHub issue and pull-request references are outside this rule on every surface.** That exemption is scoped to UAT and SPEC identifiers and does not reach them; the comment rule holds the verdict instead, and it applies to `.gaia/tests/`, `.gaia/scripts/tests/`, and `.github/audit/tests/`, which are release-excluded, as well as to the shipped `.claude/hooks/` scripts and the shipped `.github/**` helper scripts, where the release boundary adds a qualification obligation the release-excluded surfaces are exempt from.
<!-- gaia:maintainer-only:end -->
