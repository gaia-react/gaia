...
| `doc-audit-promote-source.bats`   | Doc-conformance for the `promote` action's source-side contract in `.claude/skills/gaia/references/audit.md`: the schema offers `source_action` in its three modes (`delete`, `replace`, `keep`) with the `source_before` / `source_after` pair scoped to the one mode, the apply loop checks the promote's own drift signals (which neither generic drift bullet reaches, since a promote carries neither `expect_sha256` nor a `before:`/`expect:` snippet), acts on each of the three modes, edits the source **last** so a failed wiki write leaves it intact, and fails closed on an unrecognized mode rather than guessing; post-apply verification covers the source side and verifies `replace` positively only, since `source_before` can legitimately survive elsewhere. Whole-file absence check on `delete_source_after`, because a half-applied rename leaves Stage 2 with no arm for what Stage 1 wrote. The negative half pins what the design bought: `## Ordering` keeps one flat type order with no same-path carve-out, and the `depends_on` gate stays `delete-entry`-only, so reintroducing the promote/shrink pair behind a gated shrink fails loudly. |
