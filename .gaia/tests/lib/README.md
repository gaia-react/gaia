...
| `doc-audit-remedy-set.bats`       | Doc-conformance for `/gaia-audit`'s over-budget remedy contract in `.claude/skills/gaia/references/audit.md` Step 3: the budget is per file with no aggregate over `.claude/rules/*.md`, the three remedies are evaluated independently rather than offered as a pick-one menu, each names its action shape and, for the first, the same-file `source_expect_sha256` drift that blocks it, the split routes to `## Out-of-scope findings` because no action type creates a non-wiki file, and the report template's over-budget line carries the per-remedy outcome. Forbids the original menu sentence and any issue-number citation, which would mean nothing on an adopter clone. |
