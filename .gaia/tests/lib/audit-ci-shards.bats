...
@test "W3 adversarial: a bare true aggregator step is caught" {
  ...
  local replacement=$'      - name: Require every shard and every live-tree job to have succeeded\n        run: true'
  replace_from "$WORKFLOW" "      - name: Require every shard and every live-tree job to have succeeded" "$replacement" "$doctored"
