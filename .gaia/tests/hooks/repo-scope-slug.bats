...
# only the repo-NAME half of a `-R`/`--repo` value against the home repo's
# remote names. A consumer that ACTS on the home repo inverts both
...
# `repo view --json nameWithOwner,url`. The sandbox's remote name and its home
# slug are deliberately UNRELATED (`gaia` vs `acme/widgets`), which is what
# lets a single fixture tell the two comparisons apart: a value the name-half
# comparison calls home and the slug comparison calls foreign.
...
@test "s1: a same-named sibling is foreign, where the name-half guard says home" {
  # The load-bearing fixture. The sandbox's remote is named `gaia`, so
