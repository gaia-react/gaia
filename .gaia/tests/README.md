...
|---|---|---|
| `whole-tree-invariants.sh` | CI, `audit-ci-tests.yml` via the `lib` shard | the pre-dispatch entry point: runs every check whose input is the whole tree as one set, `shell-lint.sh` and the shard partition among them, in about 40 seconds. Path-scoped selection cannot reach a whole-tree check, so this is the named set that replaces running whichever of them a loaded rule happens to mention. Its header owns the membership rule and every deliberate non-member's reason; `lib/whole-tree-invariants.bats` fails when a candidate joins neither list. |
