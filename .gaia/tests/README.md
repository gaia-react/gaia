...
| `shell-lint.sh` | CI, `shell-lint.yml` (two legs) | shellcheck gate over every tracked `*.sh` and `*.bats`, plus a bash-3.2 parse pass and five custom lints. Free, deterministic. Runs on any PR touching a shell script or bats suite. The ubuntu leg runs the whole harness; a `macos-latest` leg runs `--only bash32-parse`, the one pass a bash-5 runner can only skip. Also a member of `whole-tree-invariants.sh` above, which is what to run pre-merge. |
