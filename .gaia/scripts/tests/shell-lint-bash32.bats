#!/usr/bin/env bats
# Tests for the bash-3.2 parse pass in .gaia/tests/shell-lint.sh, the `--only`
# flag that selects it, and the gate's argument parsing.
#
# The parse pass has no separate detector to cover it, so its branches are
# asserted here directly, through the SHELL_LINT_BASH32 seam: a clean sweep, a
# parse error, a too-new interpreter that must skip LOUDLY, and an interpreter
# this pass cannot reason about, which must fail closed. Driving them through a
# stub rather than the host's real /bin/bash is what makes the suite give the
# same verdict on an ubuntu runner and on macOS -- the exact host-dependence the
# pass under test exists to surface.
#
# Split out of .gaia/scripts/tests/shell-lint.bats, which keeps the folded
