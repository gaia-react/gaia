...
create_worktree_unit() {
  ...
  # Provision the new worktree: shared-state symlinks plus typed routes. The
  # same script runs on every session start and on every entry into a worktree,
  # so a tree created here and a tree made by hand are provisioned by one
  # definition rather than two that can disagree.
  bash "$project_root/.claude/hooks/provision-worktree.sh" "$worktree_path" || true
