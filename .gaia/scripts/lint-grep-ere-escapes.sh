...
case "$GAIA_AWK_STATUS" in
  5)
    printf 'lint-grep-ere-escapes: no awk interpreter found; install mawk (macOS: brew install mawk; Debian/Ubuntu: apt-get install mawk) or ensure /usr/bin/awk is present\n' >&2
    exit 5
    ;;
  6)
    printf 'lint-grep-ere-escapes: GAIA_AWK resolved to an unsanctioned interpreter (%s); the sanctioned set is mawk and BWK one-true-awk\n' "$GAIA_AWK_IDENT" >&2
    exit 6
    ;;
esac
...
scan_file() {
  ...
  if [ "$is_bats" -eq 1 ]; then
    "$GAIA_AWK" -v file="$f" -v agreed="sSwWbB" -v is_bats=1 -v scripts_dir="$_gaia_guard_lib_dir" \
      "$GAIA_GUARD_AWK$OWN_AWK" "$f" "$f"
  else
    "$GAIA_AWK" -v file="$f" -v agreed="sSwWbB" -v is_bats=0 -v scripts_dir="$_gaia_guard_lib_dir" \
