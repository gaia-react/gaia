 ...
 *
 * Behavior (downstream chain triggers rely on the silent-success short-circuit):
 *   1. Read mentorship config. If !enabled → exit 0 silently.
 ...
 * on the happy path (downstream chain triggers expect silent success).
 */
export const computeProfile = async (
  ...
  // Short-circuit silently when mentorship is disabled. Downstream
  // chain triggers (e.g. spec_close) depend on this returning 0 quietly.
