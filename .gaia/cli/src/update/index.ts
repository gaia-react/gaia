 ...
 * `merge-audit-ci` oracle it invokes for `.gaia/audit-ci.yml` (Step 7c), and
 * the `merge-region` oracle it invokes for a declared generated region inside
 * an owned file. The skill hand-walks the per-file decision table (Step 7)
 * and field-merges `package.json` (Step 7a) itself, so the router carries no
 * generic whole-file merge command.
 *
