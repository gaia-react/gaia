...
- Detection (broad): `grep -rEn "\.gaia/cli/src/|\.gaia/cli/test-fixtures/|\.gaia/cli/__tests__/|\.specify/extensions/gaia/test/|\.claude-tests/|\.claude/rules/_internal/" CLAUDE.md .claude/ wiki/ --include="*.md" --include="*.sh" --include="*.yml"` (run from repo root). Every match outside the allowlist is a leak.
- Allowlist:
...
**Obscurity-rule leakage in shared / adopter-shipped surfaces.** The mentorship-display rule and similar privacy contracts are projected into per-machine user memory precisely so Claude is told *what not to do* without being told *where the obscured artifacts live*. Documenting the rule's existence — OR naming the file path it protects — in any file Claude reads at session start (CLAUDE.md), in slash-command bodies, or in distributed wiki pages defeats the obscurity by signposting either the contract or the artifact location.
- Detection — rule name: `grep -rn "mentorship.*display\|raw mentorship\|mentorship event file\|Claude must not display" CLAUDE.md .claude/ wiki/ --include="*.md"`
- Detection — protected path: `grep -rEn "telemetry/mentorship/events|events-.*\.jsonl|~/\.claude/projects/[^[:space:]]+/(gaia/)?telemetry/mentorship" CLAUDE.md .claude/ wiki/ --include="*.md"`
- Both greps must be empty outside the allowlist below.
