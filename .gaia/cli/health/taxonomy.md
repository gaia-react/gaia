...
- Codified in: `.gaia/release-scrub.yml` `wikilink-to-excluded` check; enumerates the known release-excluded slugs (`Release Workflow`, `Release-Notes`, `Bundle-time Scrub`, `GAIA`, `Steven Sacks`, `dashboard`, `Entities`, `Meta`, `CLI-Binary-Split`, `Forensics Triage Workflow`) plus class-regex alternatives for `wiki/meta/` dated audit artifacts (`consolidate-report-[0-9-]+`, `lint-report-[0-9-]+`) and flags any unwrapped `[[…]]` reference in shipped wiki pages. The `pattern` in `release-scrub.yml` is authoritative; this list and the smoke grep in `runbook.md` Bucket C are illustrative and must track it. New excluded slugs require updating the pattern.
