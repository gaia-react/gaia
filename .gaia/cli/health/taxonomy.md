...
**Obscurity-rule leakage in shared / adopter-shipped surfaces.** The mentorship-display rule and similar privacy contracts are projected into per-machine user memory precisely so Claude is told *what not to do* without being told *where the obscured artifacts live*. Documenting the rule's existence — or naming the files it protects — in any file Claude reads at session start (CLAUDE.md), in slash-command bodies, or in distributed wiki pages defeats the obscurity by signposting both the contract and the artifact.
- Detection: `grep -rn "mentorship.*display\|raw mentorship\|mentorship event file\|Claude must not display" CLAUDE.md .claude/ wiki/ --include="*.md" 2>&1` from repo root. Any non-empty match outside `.claude-tests/` and `.specify/extensions/gaia/test/` (smoke harnesses that legitimately verify the rule works — release-excluded, never adopter-visible) is a leak.
- Principle: per-machine user memory is the *only* surface that should describe the rule. The shared CLAUDE.md, slash commands, and wiki should not mention the rule, name the files it protects, or describe paths that might draw Claude's attention to those files. Implementing the rule (writing `.gaia/cli/src/mentorship/display-rule.ts` etc.) is fine because that source is release-excluded; describing it on adopter-shipped surfaces is the leak.
- Prior: `CLAUDE.md:29` (full bullet describing the rule + its CLI lifecycle); `.claude/commands/setup-gaia.md:196` (parenthetical naming the rule); `.claude/commands/gaia-init.md:242` (full description of "the mentorship-display rule (Claude must not surface raw mentorship event files)"); `wiki/concepts/Telemetry.md:24` (sentence stating "Claude must not display raw mentorship event files"). All four scrubbed in 8th audit fix; phrasing replaced with descriptions of the user-facing privacy contract that don't name the obscurity rule.
