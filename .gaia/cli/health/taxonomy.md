...
**Shipped `.sh`-bearing directories omitted from every leak-check scope simultaneously.** A directory of `owned` (shipped) shell scripts can fall completely outside both distribution-boundary enforcement primitives at once: the `maintainer-paths` scrub check's curated `scope:` list (`.gaia/release-scrub.yml`) and the `runtime-deps` `SCAN_GLOBS` array (`.gaia/cli/src/release/runtime-deps.ts`). This differs from either single-check scope gap above: when the same tree is missing from both, running both checks still yields zero coverage for that tree, so neither the prose-leak detector nor the runtime-dependency detector catches a defect there.

- Detection: cross-reference every `owned`, non-release-excluded `.sh`-bearing top-level directory in `.gaia/manifest.json` against BOTH `.gaia/release-scrub.yml`'s `maintainer-paths` `scope:` list AND `.gaia/cli/src/release/runtime-deps.ts`'s `SCAN_GLOBS`; flag any directory missing from either.
...
**Marker-wrapped maintainer-path grep hits.** A marker-unaware `maintainer-paths` / `monorepo-prefix` grep hit (Bucket B, or any pre-strip grep) that resolves inside a *balanced* `<!-- gaia:maintainer-only:start -->` / `<!-- gaia:maintainer-only:end -->` block in a marker-strip-scoped file (`wiki/**/*.md`, `.claude/**/*.md`, `.specify/extensions/gaia/**/*.md`) is not a leak. The bundle-time marker-strip removes the block from the staging tree before the real `release scrub` leak-check runs; Bucket C (the actual `gaia-maintainer release scrub`) is authoritative. Verify the marker pair is balanced (start before end, both present) before dismissing; an unbalanced pair does not strip and is a separate real finding.
