...
```
Orchestrator initializes .gaia/local/audit/ and creates this run's RUN_DIR (.gaia/local/audit/archived/<stamp>/; see §Audit artifacts for the exact command and prune-to-3 policy)
For cycle in 1..3:
  ...
  if clean (no open findings, Bucket D verdict A+ readiness, effective shared-fitness grade = A+; see §Termination):
    ...
    if still clean (challenger cleared, or already ran this run):
      Orchestrator removes RUN_DIR (rm -rf; whitelisted; RUN_DIR already lives under archived/, nothing to keep)
  ...
  Orchestrator checks open-finding fingerprints vs prior cycle (mechanical diff via jq; non-blocking residuals excluded, see §Termination) → if oscillation: escalate (Orchestrator preserves RUN_DIR, surfaces its path in escalation report)
...
- **False-clean refuted**: the false-clean challenger substantiates a finding on the terminal clean cycle (see §False-clean challenger). On a non-cycle-3 clean cycle the finding is injected as `action: real-fix` and the loop continues (fix + next-cycle reverify), so it is not an escalation. On cycle 3 there is no next cycle to fix-and-reverify, so the Orchestrator escalates with reason `false-clean-refuted` and preserves RUN_DIR.
...
**Outputs:** `RUN_DIR/c<N>/bucket-a.txt`
...
1. `git grep -nE "\.gaia/cli/src/|\.gaia/cli/test-fixtures/|\.gaia/cli/__tests__/|\.gaia/cli/health/|\.specify/extensions/gaia/test/|\.specify/specs/|\.gaia/tests/|\.gaia/scripts/tests/|\.github/audit/tests/|\.claude/rules/maintainers/" -- CLAUDE.md .claude/ wiki/ .gaia/statusline/ .specify/extensions/gaia/README.md .specify/extensions/gaia/commands/ .specify/extensions/gaia/lib/ .specify/extensions/gaia/rules/ .specify/extensions/gaia/templates/`
...
Triage decisions are written to `RUN_DIR/c<N>/bucket-b/triage.md`; one section per grep, listing each match and its disposition (skip / finding) with the rule that decided it.
...
**Outputs:** `RUN_DIR/c<N>/bucket-b/grep-1.txt`, `grep-2.txt`, `RUN_DIR/c<N>/bucket-b/triage.md`
...
mkdir -p "$STAGING"
# Discovery goes through the shared boundary release.yml stages through, so
# this build reads the tracked set exactly as production does. `-z` stops git's
# default core.quotePath C-quoting a non-ASCII tracked path into this list;
# rsync would then look the quoted spelling up literally and either drop the
# file from the staging tree or error. The stream is translated back to
# newlines rather than read as NUL records because both consumers below are
# newline-oriented: `grep -f` reads a newline-delimited pattern file, and macOS
# ships openrsync, which has no `--from0`. That conversion cannot represent a
# path holding a literal newline, so the boundary refuses one outright and
# names it (#1669); it also fails closed on the discovery by reading git's own
# exit status directly, rather than letting a git failure hide behind `tr`'s
# exit 0 and leave an empty list, an empty include set, and every check below
# passing having staged no files at all.
# The message names both arms because the boundary has two: a refusal (a
# tracked path holds a newline) and a failure (git, mktemp, the output write).
# The stderr line above it says which.
if ! bash .gaia/scripts/list-tracked-paths.sh . "$ALL_TRACKED"; then
...
**Outputs:** `RUN_DIR/c<N>/bucket-c.txt`
...
**Outputs:** `RUN_DIR/c<N>/bucket-d.md`
...
- **decided-not-finding** → the finding matches a "Decided / not findings" entry (in `.gaia/cli/health/taxonomy.md` or the fitness spec), or is a harness-path finding below the triage threshold (see **Triage threshold** below). **If the entry already exists**, the Adjudicator records the match and moves on (no edit, no circuit breaker); it becomes a non-blocking residual (see §Termination), retained in `findings.json` but excluded from the clean gate and the oscillation guard. **If it is a new not-a-finding class**, the Adjudicator records the proposed entry in `findings.json` rather than writing it (a leaf subagent cannot pause for human-confirm). The Orchestrator gates it on the circuit breaker, gets human-confirm, and writes the entry, after which it is likewise a non-blocking residual.

**Triage threshold.** Before assigning `real-fix` or `taxonomy-update` to a harness-path finding, the Adjudicator Reads `.claude/rules/maintainers/harness-triage-threshold.md` by path. A harness finding that meets none of its criteria gets no Fixer and no filing: record it as `decided-not-finding` with the reason `below triage threshold` and the criteria it fails (the rule is its Decided entry, so no taxonomy entry or human-confirm is needed), a non-blocking residual like any other, and list it one line under "Waived" in the report.

## False-clean challenger

The cycle loop is adversarial against the product (five buckets, a fresh per-cycle Adjudicator, cross-cycle oscillation detection), but nothing challenges its own terminal CLEAN verdict. A false-clean (a blind spot shared by every bucket, the Adjudicator dismissing a real finding as a `decided-not-finding`, or an over-graded category) exits A+ and then deletes RUN_DIR's per-cycle evidence, and oscillation cannot catch it: a clean exit has zero open findings and no next cycle to intersect against.
...
Each lens is a parallel `general-purpose` leaf the Orchestrator spawns, handed: the terminal cycle's bucket artifacts (`RUN_DIR/c<N>/bucket-a.txt`, `bucket-b/`, `bucket-c.txt`, `bucket-d.md`, `bucket-e/`), `RUN_DIR/c<N>/findings.json`, both "Decided / not findings" lists (`.gaia/cli/health/taxonomy.md` § Decided / not findings and `wiki/decisions/Claude Integration Fitness.md` § Decided / not findings), and `.claude/rules/maintainers/harness-triage-threshold.md`, so it does not re-surface settled items. Each writes `{"findings": [...]}` in the canonical finding schema (see §Audit artifacts) to `RUN_DIR/c<N>/challenger-<LENS>.json`, with an empty array when it substantiates nothing, and returns no narrative.
...
- **Misclassification (id prefix `MC`).** Always runs. For each `decided-not-finding` and `false-positive` in `findings.json`, verify it TRULY matches a taxonomy or fitness "Decided" entry, not a stretched near-match; cite the matched entry's line. A `decided-not-finding` with the reason `below triage threshold` is verified against that rule instead: it must sit on a harness path and meet none of the rule's criteria. A `decided-not-finding` that does not actually match its claimed Decided entry (a real finding dismissed as settled) is a finding.
...
> You are an ADVERSARIAL challenger of a GAIA health-audit's TERMINAL CLEAN verdict. The cycle artifacts are in `<C_DIR>`; repo root is `<repo_root>`. Read the bucket artifacts and `findings.json` first, and read the two "Decided / not findings" lists plus `.claude/rules/maintainers/harness-triage-threshold.md` (a harness finding meeting none of its criteria is settled) so you do not re-surface settled items. The loop is about to report A+ and delete the evidence. Your job is to find the reason that verdict is FALSE, not to confirm it. Cite evidence as `file:line`. Be concrete and falsifiable: a defect a fixer can act on by reading one file is a good finding, a vague "could be cleaner" is not.
>
...
- **Clean cycle IS cycle 3** (no next cycle to fix-and-reverify): do NOT inject-and-continue (there is nowhere for the fix to land). Escalate with reason `false-clean-refuted` and PRESERVE RUN_DIR (skip the clean-exit `rm -rf RUN_DIR`).
...
Per-cycle artifacts are stored under `RUN_DIR/c<N>/`, where `RUN_DIR` is this run's `.gaia/local/audit/archived/<stamp>/` folder (see §Audit artifacts lifecycle below). `.gaia/local/` is gitignored, release-excluded, and `block-rm-rf.sh`-whitelisted for these paths.
...
- **At `/health-audit` start**: Orchestrator runs `mkdir -p .gaia/local/audit; RUN_STAMP=$(date +%Y-%m-%d-%H-%M); RUN_DIR=".gaia/local/audit/archived/$RUN_STAMP"; mkdir -p "$RUN_DIR"; find .gaia/local/audit/archived -mindepth 1 -maxdepth 1 -type d 2>/dev/null | sort -r | tail -n +4 | while IFS= read -r d; do rm -rf "$d"; done`. Every run gets its own timestamped folder under `archived/` from the moment it starts, so cycle artifacts (`c1/`..`c3/`) never sit loose at the flat `.gaia/local/audit/` root waiting for a later run to archive them; they are written to `RUN_DIR/c<N>/` for the run's duration (see §Cycle loop). `archived/` is then pruned to the 3 most recent run folders (older ones deleted; the `YYYY-MM-DD-HH-MM` stamp still sorts chronologically, lexicographically). The merge-gate markers, `KNOWLEDGE-*.md` reports, the `comprehensive/` tree (its own lifecycle, tracked separately), and `archived/` itself stay at the flat root. (`worthiness.jsonl` lives under the sibling `.gaia/local/worthiness-ledger/` directory, outside this sweep's scope entirely.)
- **At cycle start**: Orchestrator creates `RUN_DIR/c<N>/` and bucket sub-dirs.
