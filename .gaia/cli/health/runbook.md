...
**Triage threshold.** Before assigning `real-fix` or `taxonomy-update` to a harness-path finding, the Adjudicator Reads `.claude/rules/maintainers/harness-triage-threshold.md` by path. A harness finding that meets none of its criteria gets no Fixer and no filing: record it as `decided-not-finding` with the reason `below triage threshold` (the threshold is its Decided entry, so no taxonomy entry or human-confirm is needed), a non-blocking residual like any other, and list it one line under "Waived" in the report.
