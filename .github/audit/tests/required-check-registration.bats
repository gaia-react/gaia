...
#   - setup-gaia-ci.md REGISTERS the check: it owns the literal
#     `required_status_checks` PUT with `contexts[]=GAIA-Audit`, run after the
#     first push once CI is being wired up.
#   - gaia-init.md DELEGATES that registration to /setup-gaia-ci rather than
...
# strings (setup-gaia-ci) and the delegation references (gaia-init).
#
...
# -----------------------------------------------------------------------------
# gaia-init recipe delegates registration to /setup-gaia-ci and inlines no
# registration command of its own.
# -----------------------------------------------------------------------------

@test "gaia-init delegates GAIA-Audit required-check registration to /setup-gaia-ci" {
  run grep -F "/setup-gaia-ci" "$GAIA_INIT"
  [ "$status" -eq 0 ]
...
# -----------------------------------------------------------------------------
# setup-gaia-ci recipe registers the GAIA-Audit status, not the job name, via
...
@test "setup-gaia-ci does not register the bare code-review-audit job name as the required check" {
