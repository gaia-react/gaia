...
- **UAT-022.** A `time_to_resolved_spec` event with `question_count: 8`, `question_ceiling: 10`, `duration_seconds: 1850`, `abandoned: false`, `area_tags`. `.gaia/cli/gaia telemetry emit time_to_resolved_spec --spec-id SPEC-300 --area-tags visual --question-count 8 --duration-seconds 1850 --question-ceiling 10 --abandoned false`.
- **UAT-023.** A `code_review_audit_finding` event with `finding_class: holistic/non-null-assertion`, `severity: warning`, `pr_number`, `area_tags`. `.gaia/cli/gaia telemetry emit code_review_audit_finding --spec-id SPEC-300 --area-tags react --pr-number 88 --finding-class holistic/non-null-assertion --severity warning --auditor-type code-review-audit`.
